...
import { debounceTime, map, merge, pairwise, startWith, switchMap, takeUntil } from 'rxjs/operators';
import {
...
export class LoginTextsComponent implements OnDestroy {
  ...
  private loadedData$: BehaviorSubject<number> = new BehaviorSubject(1);
  constructor(
  ...
  public getCurrentValues(req: any): Promise<any> {
    return (this.service as ManagementService).getCustomLoginTexts(req).then((res) => {
      if (res.customText) {
        if (res.customText.details && res.customText.details.changeDate && res.customText.details.changeDate.seconds > 0) {
