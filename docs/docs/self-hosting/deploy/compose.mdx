...
2. Make sure the containers use the latest image versions.
    ...
    docker compose pull
    ```
3. Generate a private key and public key, which you can then mount to the Zitadel API and Zitadel login so that they can communicate.
    ```shell
    ...
    openssl pkcs8 -topk8 -in login-system-user.pem -nocrypt | base64 > login-system-user-key.pem
    ```
4. Run the PostgreSQL database, the Zitadel API, and the Zitadel login.
    ```shell
    docker compose up --detach --wait
    ```
5. Verify the containers are running and healthy.
    ```shell
    docker compose ps
    ```
