...
<Tabs>
<TabItem value="dev" label="Getting Started">
...
<TabItem value="prod" label="Getting to Production">
...
- [ ] TODO: Describe what we need the master key for
...
:::warning
Do not use this login-system-user-base64.pem file in production.
:::

Write a hardcoded RSA key pair for development purposes only.
...
Generate a secure RSA key pair for production use:
...
Get started by setting the following environment variables:
...
Additionally to the environment variables described in the "Getting Started" tab, set the following variable:

```shell
# Require verified emails for users to be able to log in.
export EMAIL_VERIFICATION=true
```

Use a service manager like pm2 or SystemD to run the Login app in production.

- [ ] TODO: Test this 

```shell
npm install pm2 -g
./zitadel-login/entrypoint.sh pm2 start ./zitadel-login/apps/login/server.js
```
