...
The stack consists of three long-running containers and a couple of short-lived containers:
With this example configuration, you run a [Docker Compose](https://docs.docker.com/compose/) Zitadel and the default login at the same domain behind a Traefik reverse proxy using [Docker Compose](https://docs.docker.com/compose/).
Traefik accepts incoming requests via HTTPS.
It forwards calls to `/ui/v2/login` using plaintext HTTP to a login container.
All other paths are forwarded to the Zitadel container using HTTP/2.
The Traefik container and the login container call the Zitadel container via the internal Docker network using the Docker DNS name `zitadel` and HTTP/2.
...
The setup is tested against Docker version 28.0.1 and Docker Compose version v2.33.1
...
Open your favorite internet browser at https://127.0.0.1.sslip.io/ui/console/.
