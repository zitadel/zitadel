...
Flags:
  -h, --help                     help for mirror
      ...
      --ignore-previous          ignores previous migrations of the events table
      --replace                  replaces all data except events and projections

      --instance strings         id or comma separated ids of the instance(s) to migrate. Either this or the `--system`-flag must be set
      --system                   migrates the whole system. Either this or the `--instance`-flag must be set
...
Eventstore:
  # Limit the maximum amount of client side retries of the eventstore push command
...
* **eventstore.events2**: because only event of instances are migrated
* **projections.current_states**: If your deployment was upgraded several times it the number of entries in the destination will be lower
* **projections.locks**: If your deployment was upgraded several times it the number of entries in the destination will be lower
* **projections.keys4\***: Only not expired keys are inserted
