...
The `mirror` command allows you to do database to database migrations as well as migrations between a database and files. This functionality is useful for copying data between databases, or between a database and files.

The data can be mirrored to multiple databases without influencing each other.

## Use cases

Migrate from cockroachdb to postgres or vice versa.

Migrate from a database to files for sharing.

Replicate data to a secondary environment for testing.

## Prerequisites

If using database as source, you need an existing source database, most probably the database ZITADEL currently serves traffic from.

If using database as destination, database needs to be initialized and setup without an instance.

If using files as source, you need the following existing files: `system.assets.csv`, `system.encryption_keys.csv`, `auth.auth_requests.csv`, `eventstore.events2.csv`, and `eventstore.unique_constraints.csv`. These files all need to be in the same directory as the one set in the config file.
  ...
  -h, --help                     help for mirror
      ...
      --replace                  replaces all data of the following tables for the provided instances or all if the `--system`-flag is set:
                                 ...
                                 * eventstore.unique_constraints
                                 This should be provided if you want to execute the mirror command multiple times so that the static data are also mirrored to prevent inconsistent states.

# Both --ignore-previous and --replace flags need to be set if mirroring to/from files
...
```yaml
# The source database the data are copied from. Use either cockroach, postgres, or files. By default cockroach is used
Source:
  cockroach:
  ...
  # Files are used as soon as a path is set
  file:
    path: # /path/to/my/directory

# The destination database the data are copied to. Use either cockroach, postgres, or files. By default cockroach is used
Destination:
  cockroach:
