...
You can use these flags to treat secret configuration files differently from standard configuration files.
For example, configuration files stored in git may contain public information such as a database hostname.
To avoid storing private information — such as a database admin credential — in git, use a `--steps` flag that requests the private information from a secret manager.
