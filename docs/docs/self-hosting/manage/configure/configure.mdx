...
This guide assumes you are familiar with [running ZITADEL using the most minimal configuration possible](/docs/self-hosting/deploy/overview).
...
For a description of all _runtime configuration_ options and their defaults, read the [ZITADEL source code](https://github.com/zitadel/zitadel/blob/main/cmd/defaults.yaml).
...
Use these flags to treat secret configuration files differently from standard configuration files.
For example, configuration files stored in `git` may contain public information such as a database hostname.
To avoid storing private information — such as a database admin credential — in `git`, use a `--steps` flag that requests the information from a secret manager.
