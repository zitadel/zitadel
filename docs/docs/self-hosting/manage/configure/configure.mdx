...
The proxy can be configured using the env variables `HTTP_PROXY` for http requests and `HTTPS_PROXY` for HTTPS requests. The variables are used as proxy url.
