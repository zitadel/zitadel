...
In this example we will use a service account with a [personal access token](https://zitadel.com/docs/guides/integrate/service-users/private-access-token) which can be used directly to access the example API.
...
1. Create a service user and a Personal Access Token (PAT) for that user by following [this guide](https://zitadel.com/docs/guides/integrate/service-users/private-access-token#create-a-service-user-with-a-pat). 
