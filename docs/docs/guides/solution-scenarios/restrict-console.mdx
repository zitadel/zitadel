...
ZITADEL includes a console that allows Administrators to configure all resources. All uses, including end-users, by default, view and manage their profile information.
...
- Only administrators should be able to access Console to manage their users
...
If you are running ZITADEL self-hosted, we recommend restricting the access to ZITADEL Console via WAF/reverse-proxy for Non-Administrator users
...
Start by granting the organization of your administrators, that should be able to access the Console, the [default project](/docs/guides/manage/console/projects-overview#the-default-project):
...
This means either you add the administrator to the organization or you use a manager on the instance level with "IAM-OWNER" permissions.
After that create a Personal Access Token (PAT) for the administrator.

More detailed information about creating a PAT and administrator roles you can find [here](/docs/guides/integrate/service-users/personal-access-token)
