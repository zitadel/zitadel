...
### Restricting Console access in notification messages

Zitadel notifications template has a button that by default redirect users to the Zitadel console to perform an action (e.g verify an email, enter an init code...)

You can avoid this redirect, setting which URL should be used for these buttons.

You've two options that can be used combined or separately:

- Use the default redirect URI for all notification messages buttons. Read more in [Settings - Default Redirect URI](/docs/guides/manage/console/default-settings#default-redirect-uri)
- In the [**Message Texts**](#./default-settings#message-texts) section, you can also define the URL for specific messages and languages using the ButtonUrl text area. Note that the ButtonUrl value will override the use of the default redirect URI for the specified combination of message and language.

<img
  src="/docs//img/guides/solution-scenarios/set_buttonurl_in_message_texts.png"
  alt="Message Texts: specify a button url"
  width="600px"
/>
