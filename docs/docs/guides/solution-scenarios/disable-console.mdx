...
sidebar_label: Disable ZITADEL Console
---

ZITADEL does include the ZITADEL Console, which allows you to manage all ZITADEL resources as well as end users to manage their profile information.
...
At the moment it is not possible to simply disable the Console but with a workaround you can prevent users from accessing the Console.
...
The Default Redirect URI will be used, if a user calls the login page directly. More specifically, typically a client will initiate login with an auth request.
...
With this workaround, you will limit users from accessing the default-project, if they are not explicitly granted to use this project.
...
Start by granting the organization of your Managers that should be able to access the Console the "ZITADEL" project:
...
Where
- CUSTOM_DOMAIN is your domain, f.e. b2b-demo-rbxajm.zitadel.cloud
...
- PROJECT_NAME is the default-project's name as shown in the Console
