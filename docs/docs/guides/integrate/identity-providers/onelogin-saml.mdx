...
![Image: OneLogin Add Apps screen showing SAML connector selection](/img/guides/onelogin-add-apps.png)
...
![Image: OneLogin SAML configuration with ZITADEL URLs](/img/guides/onelogin-configuration.png)
...
3. Add or edit parameters so that at least the following are sent and **included in the SAML assertion**:
   ...
   - `firstName` → mapped to the user’s given name (for example **First Name**).
   - `lastName` → mapped to the user’s family name (for example **Last Name**).
...
![Activate OneLogin SAML provider in ZITADEL](/img/guides/zitadel-activate-onelogin-saml.png)
...
![Image: OneLogin button on ZITADEL login](/img/guides/zitadel-login-onelogin-button.png)
