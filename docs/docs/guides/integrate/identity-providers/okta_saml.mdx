...
### Create a new SAML SP Provider

To be able to create the application in OKTA we need the provider id from ZITADEL.
1. Create a new SAML SP provider with a name and a random text in the Metadata Xml field.
...
`$CUSTOM-DOMAIN/ui/console/org/provider/saml/257382385664925915`

As an alternative you can add the SAML identity provider trhough the API, either on the default settings or on a specific organization:
...
4. Fill the configuration as following (Replace `your-domain` and `saml-idp-id` with your data):
 ...
 - Example redirect url for the domain `https://acme-gzoe4x.zitadel.cloud` would look like this:  `https://acme-gzoe4x.zitadel.cloud/idps/257372385775925924/saml/metadata`
...
If you prefere changing the configuration through the API you can update the SAML provider on the default settings or a specific orgnaization:
...
:::note
At the moment it is not possible to autofill the user data in ZITADEL with the data from OKTA.
This means the user has to fill the information manually.
:::