...
This configuration is based on the generic OIDC configuration.

:::

## Register an external identity provider

In this step we will add a new OKTA identity provider to federate identities with ZITADEL.
...
2. Make sure you add the ZITADEL callback redirect uris
   - {custom-domain}/ui/login/login/externalidp/callback
