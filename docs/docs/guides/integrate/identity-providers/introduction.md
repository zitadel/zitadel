...
If you use the Generic OIDC provider or the Generic OAuth provider note that the use of [Proof Key for Code Exchange](https://datatracker.ietf.org/doc/html/rfc7636) for authorization requests is enabled by default to mitigate the authorization code interception attack.

This setting can be disabled in the Optional settings section when you create or update these providers settings:

![Identity Provider Overview](/img/guides/zitadel_idp_pkce.png)
