...
### Required when using Login V2: Use Actions V2 to map the required fields for user creation
Creation of users in ZITADEL will fail if the required fields to create a user are not set.
...
The following minimal example modifies the response `/zitadel.user.v2.UserService/RetrieveIdentityProviderIntent` to set the required fields
...
### Optional when using Login V1: Use Actions V1 to optionally prefill user data
