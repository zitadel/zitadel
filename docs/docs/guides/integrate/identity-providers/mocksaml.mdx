...
They are available under *https://*`ZITADEL_DOMAIN`*/idps/*`ZITADEL_PROVIDER_ID`*/saml/metadata*, but this step can be skipped due to the fact that MockSAML is only for testing purposes.
