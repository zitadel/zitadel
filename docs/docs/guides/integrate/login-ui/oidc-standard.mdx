...
[Get Auth Request By ID Documentation](/docs/reference#tag/openid-connect-oidc_v2/oidc-service-get-auth-request)
...
Implement all the steps you like the user the go trough by [creating](/docs/reference#tag/sessionssession-service-create-session) and [updating](/docs/apis/resources/session_service/session-service-set-session) the user-session.
...
Read more about the [Finalize Auth Request Documentation](/docs/reference#tag/openid-connect-oidc_v2/oidc-service-create-callback)
...
Prompt the user to select a session, terminate it using the [corresponding endpoint](/docs/reference#tag/sessionssession-service-delete-session) and send the user to the `post_logout_redirect` URL.
