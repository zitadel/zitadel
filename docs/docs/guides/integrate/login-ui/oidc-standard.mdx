...
Then end session endpoint has to be implemented as all to other OIDC endpoints. This means you have to proxy the request from you UI to the ZITADEL.
To be able to redirect the user to the correct URLs after a successful logout, you need the following endpoint:

```/logout?post_logout_redirect="```

You will get the post logout redirect URLs in the parameter and can send the user to that URL.