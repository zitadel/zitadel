...
The login is then able to load an [AuthRequest](/docs/reference#tag/openid-connect-oidc_v2/oidc-service-get-auth-request#get-oidc-auth-request-details).
...
If enough user information is retrieved and the user is authenticated according to the policies, the flow is finalized by [requesting a the callback url](/docs/reference#tag/openid-connect-oidc_v2/oidc-service-create-callback) for the auth request and the user is redirected back to the application.
...
To register your login domain on your instance, [add](/docs/reference/deprecated#tag/admin-service/POST/admin/v1/trusted_domains) your domain on your instances trusted domains.
...
Also ensure your login domain is registered on your instance by adding it as a [trusted domain](/docs/reference/deprecated#tag/admin-service/POST/admin/v1/trusted_domains).
