...
The confidential side of the application is autenticated with a service user.
It fetches policies and settings from your instance and is able to autenticate users without a OIDC context to allow them to self service their account.
...
The login requires to register its host as custom domain of your instance. We refer to [installing domain](/docs/apis/resources/system/system-service-add-domain) for self hosters or the [guide for cloud users](/docs/guides/manage/cloud/instances#add-custom-domain), to setup domains.
You can read more information about it in [how to setup domains](./typescript-repo#how-to-setup-domains).

The backend side of the application is authorized using a machine user and a personal access token.
...
The /loginname page allows to enter loginname or email of a user. User discovery is implemented at /api/loginname and if the user is found, redirected to the available authentication method page.
...
If enough user information is retrieved and the user is authenticated, the flow is finalized by [requesting a the callback url](/docs/apis/resources/oidc_service/oidc-service-create-callback) for the auth request and the user is redirected back to the application.
...
After authenticating, or with a valid session, users are able to self service their account.
...
Make sure the host of the login is set in your instance.
To detect the proxy, `x-zitadel-login-client` and `x-zitadel-forwarded` must be set as request headers.
`x-zitadel-login-client` is the ID of the service user, `x-zitadel-forwarded` falls back to the host.
