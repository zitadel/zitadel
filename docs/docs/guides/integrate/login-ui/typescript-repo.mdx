...
The UI shows branding and behaves based on the policies and settings your instance or organization (when using an org scope) has.
It is also able to autenticate users without an OIDC context by navigating to the host of the login, to allow them to self service their account.
It's set of features will increase with time and its progress will be tracked [here](./typescript-repo#implemented-features).
...
_Note that the illustration is just a representation of the architecture and may not specify actual values._
