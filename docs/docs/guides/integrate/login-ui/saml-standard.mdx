...
Which should result in a POST call to the url 'https://myapp.example.com/acs' with the payload with the following attributes:
...
Which should result in a redirect to the url 'https://myapp.example.com/acs?RelayState=...&SAMLResponse=...' with all information as URL parameters.
