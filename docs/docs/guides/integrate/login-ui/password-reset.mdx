...
Make sure to also include the URL Template so ZITADEL knows where to redirect the user when clicking on the reset link in the email.
...
```bash
curl --request POST \
  --url https://$ZITADEL_DOMAIN/v2alpha/users/218662596918640897/password_reset \
...
In this case it requires additionally the old password instead of the verification code.
:::
...
```bash
curl --request POST \
  --url https://$ZITADEL_DOMAIN/v2alpha/users/218662596918640897/password \
  --header 'Accept: application/json' \
  --header 'Authorization: Bearer '"$TOKEN"'' \
  --header 'Content-Type: application/json' \
  --data '{
