...
By default, all of these login methods are allowed and can be adjusted by [Administrators](/docs/concepts/structure/administrators) to meet their requirements.
...
1. **Secure Tenant Isolation**: Ensures robust security measures to prevent unauthorized access between tenants, maintaining data privacy and compliance. [Administrators](/docs/concepts/structure/administrators) of an organization have only access to data and configuration within their Organization.
...
3. **Centralized Administration**: Provides [centralized administration](/docs/guides/manage/console/administrators) for efficient management across all tenants.
...
The [passkeys](/docs/concepts/features/passkeys.md) login flow follows the FIDO2 / WebAuthN standard.
