...
:::caution
Before you reconfigure Zitadel, we recommend you to create a service user with a personal access token and the IAM_OWNER role for a lockout-failsafe.
:::

<Tabs queryString="deployment">
    <TabItem value="zitadel_cloud_standard" label="Zitadel Cloud Standard V2 Login" default="true">

        Zitadel Cloud provides the standard V2 login at the fixed path `/ui/v2/login`. To activate it in your Zitadel Cloud instance, you can follow one of these steps:
