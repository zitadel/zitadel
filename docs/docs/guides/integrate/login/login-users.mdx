...
Available second factor (2FA) and multi-factor authentication methods (MFA) include OTP via authenticator app, TOTP via SMS, OTP via email, and U2F.
...
:::info Session API support for OIDC amd SAML
...
Main reasons on why developers might want to build their own login UI include:
