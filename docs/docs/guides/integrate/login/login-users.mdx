...
We make sure to harden the login UI an minimize the attack surface.
...
Available second-factor (2FA) and multi-factor authentication methods (MFA) include OTP via an authenticator app, TOTP via SMS, OTP via email, and U2F.
