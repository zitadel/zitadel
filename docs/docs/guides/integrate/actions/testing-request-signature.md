...
In this guide, you will create a ZITADEL execution and target. Before a user is created through the API, the target is called, with checked signature.
...
What happens here is only a target which prints out the received request, and checks the signature beforehand.
