...
See a description of all possible _runtime configuration_ options with their defaults [in the source code](https://github.com/zitadel/zitadel/blob/v2/cmd/defaults.yaml).
The `zitadel` binary expects the `--config` flag for this configuration.

### Database Initialization
Apart from these options, ZITADEL uses a [different configuration](https://github.com/zitadel/zitadel/blob/v2/cmd/adminn/setup/steps.yaml) for _database initialization steps_.
...
- Also, you should configure a [different default IAM admin username and password](iam-admin-user).

:::caution

<!-- TODO: Better mark the link in the UI -->
The ZITADEL management console [requires end-to-end HTTP/2 support](./http2)
