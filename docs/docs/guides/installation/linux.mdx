...
```bash
curl -s https://api.github.com/repos/zitadel/zitadel/releases/tags/v2.0.0-v2-alpha.17 | grep "browser_download_url.*zitadel_Linux_$(uname -i)" | cut -d '"' -f 4 | wget -i - -O - | sudo tar -x -z -v -C /usr/local/bin -f - zitadel && sudo chmod +x /usr/local/bin/zitadel && sudo chown $(id -u):$(id -g) /usr/local/bin/zitadel
```
