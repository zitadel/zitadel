...
## Domain verification and Organization Domain
...
If the "validate organization domains" settings in the [Domain Settings](./default-settings#domain-settings) is set to true, you have to prove the ownership of your domain, by DNS or HTTP challenge.
If the setting is set to false, the created domain will automatically be set to verified.

An organization can have multiple domain names, but only one domain is the Organization Domain.
The Organization Domain defines which login name ZITADEL displays to the user, and what information gets asserted in access_tokens (`preferred_username`).
...
- [**Organization Domains**](/docs/guides/manage/console/organizations-overview#usernames-and-domains): This is where you manage your organization specific domains which can be used to build usernames
