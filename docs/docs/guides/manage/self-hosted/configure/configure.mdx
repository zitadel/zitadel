...
Prefix the key with <em>ZITADEL\_</em>, uppercase the propery and join sections by an underscore \_.
For example, if you want to configure the default ZITADEL IAM admin username and password, make sure the ZITADEL binary runtime has the variables <em>ZITADEL_FIRSTINSTANCE_ORG_HUMAN_USERNAME</em> and <em>ZITADEL_FIRSTINSTANCE_ORG_HUMAN_PASSWORD</em> set.
