...
Prefix the key with _ZITADEL\__, uppercase the propery and join sections by an underscore \_.
For example, if you want to configure the default ZITADEL IAM admin username and password, make sure the ZITADEL binary runtime has the variables _ZITADEL_FIRSTINSTANCE_ORG_HUMAN_USERNAME_ and _ZITADEL_FIRSTINSTANCE_ORG_HUMAN_PASSWORD_ set.
