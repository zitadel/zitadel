...
[Actions](/docs/apis/actions/introduction) allow you to extend ZITADEL with custom code to change default behaviors or call external systems.
...
ZITADEL APIs were originally organized by UseCase/Context, such as Auth API for authenticated users and Management API for organization managers.
...
The management API is the interface where systems can mutate IAM objects like organizations, projects, clients, users and so on if they have the necessary access rights.
...
The Assets API allows you to up- and download all kinds of assets. These can be files such as logos, fonts or user avatar.
