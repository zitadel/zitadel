...
[Actions](/docs/guides/manage/console/actions-overview) allow extending ZITADEL with custom code to change default behaviors or calling external systems.
...
For user authentication on devices with limited accessibility (eg, SmartTV, Smartwatch, etc.) use the [device authorization grant](/docs/guides/integrate/login/oidc/device-authorization).
