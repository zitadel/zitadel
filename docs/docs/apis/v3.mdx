...
- For pagination, [we provide a *next_page_token*](https://protobuf.dev/programming-guides/api/#define-pagination-api) in the response. <!-- TODO verify -->
...
- For all resources, custom metadata can be added and removed.
- All resources are queryable by their metadata.
...
Additional methods for instances:
...
- GetOIDCInformation (replaces the management services GetIAM method <!-- TODO verify --> )
...
CRUD for context-aware domains
...
- Standard CRUD for Users <!-- TODO verify -->
- All methods from the auth service that are not defined in this document already (list and revoke tokens <!-- TODO verify -->, list permissions etc.)

### ZITADELUserSchemas

- Standard CRUD and methods for reusable UserSchemas <!-- TODO verify @stebenz, @adlerhurst-->

### ZITADELSessions

Additional method Challenge which returns an updated session. <!-- TODO verify @livio-a -->
...
Additional methods for memberships:

- ListAvailableRoles (context-aware)

### ZITADELGrants

- CRUD for project grants
- CRUD for user grants
- CRUD for roles

### ZITADELActions

- standard CRUD for context-aware Executions <!-- TODO verify @stebenz -->
- standard CRUD and methods for reusable Targets <!-- TODO verify @stebenz -->

### ZITADELIdentityProviders (reusable)

Standard CRUD and methods for all IDPs as reusable resources

### ZITADELSMTPProviders (reusable)

Standard CRUD and methods for reusable resources, but only on the system and instance level

### ZITADELSMSProviders (reusable)
...
<!-- TODO should we integrate this into the user schema? @livio-a, @adlerhurst -->
...
previously known as password complexity and password age policy (do we still need the password age policy? <!-- TODO verify @livio-a -->)
