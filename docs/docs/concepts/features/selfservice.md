...
- `Administrators` are users with a [special administrator role](../../guides/manage/console/administrators) within ZITADEL and can perform administrative actions such as system configuration or granting access rights to users.

All self-service interfaces are available in different [languages](/guides/manage/customize/texts#internationalization--i18n).

:::info
ZITADEL covers the typical "CIAM" self-service capabilities as well as delegated access management for multi-tenancy scenarios. Please refer to the section [Administrators](#managers).
:::
...
It is important to note that an `Administrator` is not simply an administrative user, but can be used to create much more advanced scenarios such as delegating administration of a whole organization to a user, acting then as an administrator who manages permissions of that user group.
...
Administrators can be assigned to both human users and service users, e.g., for managing certain tasks programmatically.
