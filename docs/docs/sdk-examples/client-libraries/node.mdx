...
try {
  ...
  if (e instanceof ApiException) console.error("Error:", e.message);
}
```
...
All API failures throw an `ApiException` with a human-readable message.
Network/transport issues throw native errors.
...
The SDK’s **major** version tracks the ZITADEL core **major** to signal
compatibility. See the repo’s `VERSIONING.md` for policy and cadence.

### Contributing

This repository is autogenerated. Please open issues for bugs/feature requests.

### Reporting Issues

Open an issue in the [issue tracker](https://github.com/zitadel/client-nodejs/issues). Include:
...
- Community: [ZITADEL Discord](https://zitadel.com/chat)
- Cloud & Enterprise: see support channels in the service description
