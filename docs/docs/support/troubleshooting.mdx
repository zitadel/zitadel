...
You must add the scope *urn:zitadel:iam:org:project:id:`PROJECT_ID`:aud* to the auth request that is send from the front end.
Replace *`PROJECT_ID`* with the projectId of your backend.
...
Instead of `urn:zitadel:iam:org:project:id:zitadel:aud` you could also use *urn:zitadel:iam:org:project:id:`PROJECT_ID`:aud*, where *`PROJECT_ID`* is the projectId of the Project "ZITADEL".
