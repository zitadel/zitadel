...
app.get("/docs/img/api/me/messages", auth.require(), async (c) => {
  const user = await User.me();
...
Please follow [this guide here](/docs/guides/integrate/token-introspection/private-key-jwt), ignoring the first step as we already have the `.json`-key-file from the serviceaccount.
