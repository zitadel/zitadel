...
1. Download the `docker-compose.yaml` file from the Zitadel repository:
   ```shell
   curl -L https://raw.githubusercontent.com/zitadel/zitadel/main/docs/operate/deploy/docker-compose.yaml -o docker-compose.yaml
   ```
...
- **Use files for secrets instead of environment variables, and don't commit them to Git**: Learn how to [configure the platform](/docs/operate/configure/configure)
- **Run the containers as non-root users**: Remove the `user: "0"` lines from the `docker-compose.yaml` file.
- **Configure a different external domain or IP**: Beware that the login only works with HTTPS on non-localhost domains. [Run and configure a reverse proxy](/docs/operate/configure/proxy/reverse_proxy)
- **Serve the API and the UI together on the same port**: [Run and configure a reverse proxy](/docs/operate/configure/proxy/reverse_proxy)
- **Encrypt Traffic**: [Run and configure a reverse proxy](/docs/operate/configure/proxy/reverse_proxy)

For more detailed recommendations, read the [Zitadel Production Setup Guide](/docs/operate/operations/production).
...
<details>
    ...
    <ol>
        ...
        <li><a href="/docs/operate/operations/login-client#create-login-client">Manually create a login client personal access token (PAT) for the now running v2 login</a>.</li>
        <li>Move the PAT to `./login-client.pat`</li>
        <li>Restart the login process: `docker compose restart login`</li>
        <li><a href="/docs/operate/operations/login-client#require-login-v2">Enable the Login UI for all users</a></li>
    </ol>
</details>
