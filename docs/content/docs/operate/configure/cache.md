...
For a full configuration reference, please see the [runtime configuration file](/docs/operate/configure/configure#runtime-configuration-file) section's `defaults.yaml`.
...
**For example**: A ZITADEL deployment with 2 servers is serving 1000 req/sec total. The installation only has one instance[^1]. There is only a small amount of data cached (a few kB) so duplication is not a problem in this case. It is acceptable for [instance level setting](/docs/manage/policies/default-settings) to be out-dated for a short amount of time. When the memory cache is enabled for the instance objects, with a max age of 1 second, the instance only needs to be obtained from the database 2 times per second (once for each server). Saving 998 of redundant queries. Once an instance level setting is changed, it takes up to 1 second for all the servers to get the new state.
...
All HTTP and gRPC requests sent to ZITADEL receive an instance context. The instance is usually resolved by the domain from the request. In some cases, like the [system service](/docs/reference/api-v1/system/zitadel.system.v1.SystemService.GetInstance), the instance can be resolved by its ID. An instance object contains many of the [default settings](/docs/manage/policies/default-settings):

- Instance [features](/docs/manage/policies/default-settings#features)
- Custom domains: generated and [custom](/docs/manage/cloud/instances#add-custom-domain)
- [Trusted domains](/docs/reference/api-v1/admin/zitadel.admin.v1.AdminService.AddInstanceTrustedDomain)
- Security settings ([IFrame policy](/docs/build-and-integrate/patterns/configurations#embedding-zitadel-in-an-iframe))
- Limits[^2]
- [Allowed languages](/docs/manage/policies/default-settings#languages)
...
Milestones are used to track the administrator's progress in setting up their instance. Milestones are used to render *your next steps* in the [console](/docs/manage/console-overview) landing page.
...
Most resources like users, project and applications are part of an [organization](/docs/learn/structure/organizations). Therefore many parts of the ZITADEL logic search for an organization by ID or by their primary domain.
...
[^1]: Many deployments of ZITADEL have only one or few [instances](/docs/learn/structure/instance). Multiple instances are mostly used for ZITADEL cloud, where each customer gets at least one instance.
