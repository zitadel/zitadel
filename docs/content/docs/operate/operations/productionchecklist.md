...
- [ ] High Availability for ZITADEL containers
  ...
  - [ ] Split `zitadel init` and `zitadel setup` for fast start-up times when [scaling](/docs/operate/operations/updating_scaling) ZITADEL
...
- [ ] High Availability for critical infrastructure components (depending on your setup)
  - [ ] Loadbalancer
  - [ ] [Reverse Proxies](/docs/operate/configure/proxy/reverse_proxy)
  - [ ] Web Application Firewall

#### Networking

- [ ] Use a Layer 7 Web Application Firewall to secure ZITADEL that supports **[HTTP/2](/docs/operate/configure/http2)**
  - [ ] Limit the access by IP addresses if needed
  - [ ] Secure the access by rate limits for specific endpoints (e.g. API vs frontend) to secure availability on high load. See the [ZITADEL Cloud rate limits](/docs/manage/legal/policies/rate-limit-policy) for reference.
  - [ ] Check that your firewall also filters IPv6 traffic

### ZITADEL configuration

- [ ] Configure a valid [SMTP Server](/docs/manage/policies/default-settings#smtp) and test the email delivery
- [ ] Add [Custom Branding](/docs/build-and-integrate/customization/general/branding) if required
- [ ] Configure a valid [SMS Service](/docs/manage/policies/default-settings#sms) such as Twilio if needed
- [ ] Configure your privacy policy, terms of service and a help Link if needed
- [ ] Keep your [masterkey](/docs/operate/configure/configure) in a secure storage
...
- [ ] Ensure that your ZITADEL does not use [the default, example or _easy-to-guess_ credentials](/docs/operate/configure/database/database#zitadel-credentials)
- [ ] Use a FQDN and a trusted valid certificate for external [TLS](/docs/operate/configure/http2) connections
...
- [ ] ZITADEL [metrics](/docs/operate/operations/observability/metrics)
