...
As soon as you successfully deployed ZITADEL as a proof of concept using one of our [deployment guides](/docs/operate/deploy/overview),
...
like [Kubernetes](/docs/operate/deploy/kubernetes).
...
Read more about separating the init and setup phases on the [Updating and Scaling page](/docs/operate/operations/updating_scaling).

## Configuration

Read [on the configure page](/docs/operate/configure/configure) about the available options you have to configure ZITADEL.
...
- To make ZITADEL available at the domain of your choice, [you need to configure the ExternalDomain property](/docs/operate/configure/custom-domain).
- To enable and restrict access to **HTTPS**, head over to [the description of your TLS options](/docs/operate/configure/tls_modes).
- If you want to front ZITADEL with a reverse proxy, web application firewall or content delivery network, make sure to support **[HTTP/2](/docs/operate/configure/http2)**.
- You can also refer to some **[example reverse proxy configurations](/docs/operate/configure/proxy/reverse_proxy)**.
...
By default, [**metrics**](/docs/operate/operations/observability/metrics) are exposed at /debug/metrics in OpenTelemetry (otel) format.
...
<Callout type="info" title="ZITADEL v2 supports [CockroachDB](https://www.cockroachlabs.com/) and [PostgreSQL](https://www.postgresql.org/). Please refer to [the mirror guide](cli/mirror) to migrate to postgres.">
...
Depending on your environment, you maybe would want to tweak some settings about how ZITADEL interacts with the database in the database section of your ZITADEL configuration. Read more about your [database configuration options](/docs/operate/configure/database/database).
...
You also might want to configure how [projections](/docs/learn/event-sourcing/implementation#projections) are computed. These are the default values:
...
[ZITADEL is event sourced](/docs/learn/event-sourcing/overview).
...
- You can configure instance defaults in the DefaultInstance section.
  If you plan to eventually create [multiple virtual instances](/docs/learn/structure/instance#multiple-virtual-instances), these defaults take effect.
...
- If you don't want to use the DefaultInstance configuration for the first instance that ZITADEL automatically creates for you during the [setup phase](/docs/operate/configure/configure#database-initialization-file), you can provide a FirstInstance YAML section using the --steps argument.
- Learn how to configure ZITADEL via the [Console user interface](/docs/manage/console-overview).
- Probably, you also want to [apply your custom branding](/docs/build-and-integrate/customization/general/branding), [hook into certain events](/docs/build-and-integrate/customization/general/behavior), [customize texts](/docs/build-and-integrate/customization/general/texts) or [add metadata to your users](/docs/build-and-integrate/customization/general/user-metadata).
- If you want to automatically create ZITADEL resources, you can use the [ZITADEL Terraform Provider](/docs/reference/terraform/terraform-provider).

## Limits and Quotas

If you host ZITADEL as a service,
you might want to [limit usage and/or execute tasks on certain usage units and levels](/docs/operate/configure/usage_control).
...
<Callout type="info" title="Password hashing">
