...
- We recommend using the [Authentication Code Flow](/docs/references/openidoauth/grant-types#authorization-code) with [Proof Key for Code Exchange (PKCE)](/docs/references/openidoauth/grant-types#authorization-code) for secure OIDC configuration. This setup includes specific App, Grant, and Response Types, as well as an Authentication Method. More about the different app types can be found [here](/docs/build-and-integrate/login/oidc/oauth-recommended-flows#different-client-profiles).
...
[Skip optional steps](#7-create-an-application-in-your-project) and jump directly to the obtain ClientId and OIDC endpoints.
...
- project_resource_id (To add a ZITADEL project scope. `urn:zitadel:iam:org:project:id:[projectId]:aud` and `urn:zitadel:iam:org:projects:roles` [scopes](/docs/references/openidoauth/scopes#reserved-scopes).)
- prompt ([the OIDC prompt parameter](/docs/references/openidoauth/endpoints#additional-parameters))
