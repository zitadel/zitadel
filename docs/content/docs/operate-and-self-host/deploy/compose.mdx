...
1. Download the `docker-compose.yaml` file from the Zitadel repository:
   ```shell
   curl -L https://raw.githubusercontent.com/zitadel/zitadel/main/docs/docs/operate-and-self-host/deploy/docker-compose.yaml -o docker-compose.yaml
   ```
...
- **Use files for secrets instead of environment variables, and don't commit them to Git**: Learn how to [configure the platform](/docs/operate-and-self-host/manage/configure)
