...
import DefaultsYamlSource from '../../../../../../cmd/defaults.yaml?raw';
import StepsYamlSource from '../../../../../../cmd/setup/steps.yaml?raw';
import SkipSuperUserDatabaseQueries from './example-skip-superuser-queries.yaml?raw';

This guide assumes you are familiar with [running ZITADEL using the least amount of configuration possible](/docs/operate-and-self-host/deploy/overview).
...
<Callout type="tip" title="For overwriting the default configuration for the first instance created by zitadel setup, use the FirstInstance section in the [database initialization file](#database-initialization-file).">
...
<Callout type="tip" title="By using the FirstInstance section, you can overwrite the [DefaultInstance configuration](#runtime-configuration-file) for the first instance created by zitadel setup.">
...
- Read more about [the login process](/docs/build-and-integrate/login/login-users).
