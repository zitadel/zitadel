...
- [ ] High Availability for ZITADEL containers
  ...
  - [ ] Split `zitadel init` and `zitadel setup` for fast start-up times when [scaling](/docs/operate-and-self-host/manage/updating_scaling) ZITADEL
...
- [ ] High Availability for critical infrastructure components (depending on your setup)
  - [ ] Loadbalancer
  - [ ] [Reverse Proxies](/docs/operate-and-self-host/manage/reverseproxy/reverse_proxy)
  - [ ] Web Application Firewall

#### Networking

- [ ] Use a Layer 7 Web Application Firewall to secure ZITADEL that supports **[HTTP/2](/docs/operate-and-self-host/manage/http2)**
  - [ ] Limit the access by IP addresses if needed
  - [ ] Secure the access by rate limits for specific endpoints (e.g. API vs frontend) to secure availability on high load. See the [ZITADEL Cloud rate limits](/docs/manage-and-govern/legal/policies/rate-limit-policy) for reference.
  - [ ] Check that your firewall also filters IPv6 traffic

### ZITADEL configuration

- [ ] Configure a valid [SMTP Server](/docs/manage-and-govern/console/default-settings#smtp) and test the email delivery
...
- [ ] ZITADEL [metrics](/docs/references/observability/metrics)
