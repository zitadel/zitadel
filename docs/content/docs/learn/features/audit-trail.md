...
ZITADEL only ever appends data in an [Eventstore](/docs/learn/event-sourcing/overview), keeping all historic record.
...
<Callout type="info" title="Future Plans">
...
You can check the last changes of most objects in the [Console](/docs/manage/console-overview).
In the following screenshot you can see an example of last changes on an [user](/docs/manage/org-management/users).
...
Administrators can see all events across an instance and filter them directly in [Console](/docs/manage/console-overview).
...
- [Event API Guide](/docs/reference/api/guides/event-api)
- [API Documentation](/docs/reference/api-v1/admin/zitadel.admin.v1.AdminService.ListEvents)

Access to the API is possible with a [Service User](/docs/build-and-integrate/m2m/authenticate-service-users) account, allowing you to integrate the events with your own business logic.

## Using logs in external systems

You can use the events from the audit log in external systems such as a SOC/SIEM solution.

Follow our guide on how to [integrate ZITADEL with external systems for streaming events and audit logs](/docs/operate/operations/observability/external-audit-log).
