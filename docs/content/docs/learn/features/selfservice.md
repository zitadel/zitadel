...
- `Managers` are users with a [special manager role](/docs/manage/org-management/managers) within ZITADEL and can perform administrative actions such as system configuration or granting access rights to users.

All self-service interfaces are available in different [languages](/docs/build-and-integrate/customization/general/texts#internationalization-i18n).
...
Read more about [Service Users](/docs/build-and-integrate/m2m/authenticate-service-users) and recommended [OpenID Connect Flows](/docs/build-and-integrate/login/oidc/oauth-recommended-flows#different-client-profiles).

## Logout

Users can terminate the session for all their users (logout).
A client can also implement this, by calling the [specific endpoint](/docs/reference/openidoauth/endpoints#end-session-endpoint).
...
A list of [Manager Roles](/docs/manage/org-management/managers#roles) is available with a description of permissions.
...
In a setup like described in the [B2B Scenario](/docs/build-and-integrate/patterns/b2b), there exists an organization of the project owner and a customer organization.
