...
<Callout type="info" title="We use the terms secret and credentials interchangeable to keep this guide lean.">
...
<Callout type="info" title="The key used to encrypt and decrypt the secrets in the ZITADEL database is called `masterkey` and needs to be exactly 32 bytes long.">
...
<Callout type="info" title="Due to the inherent nature of a public key being public we safeguard them against malicious key changes with our unique [eventstore concept](../eventstore/overview).">
...
<Callout type="info" title="ZITADEL hashes all Passwords and Client Secrets in an non reversible way to further reduce the risk of a Secrets Storage breach.">
...
<Callout type="info" title="ZITADEL updates stored hashes when the configured algorithm or its parameters are updated,">
...
Further `Verifiers` must be enabled in the [configuration](/docs/operate/configure/configure) by the system administrator. 
...
<Callout type="info" title="By default ZITADEL uses `RSA256` for signing purposes and `AES256` for encryption">
...
You find [here the many ways how ZITADEL can consume the Masterkey](/docs/operate/configure/configure).

### TLS Material

ZITADEL does support end to end TLS as such it can consume TLS Key Material.
Please check our [TLS Modes documentation](/docs/operate/configure/tls_modes) for more details.

### Admin User

The initial Admin User of ZITADEL can be configured through [ZITADELs config options](/docs/operate/configure/configure).
