...
![JWT authentication method](/go/api-select_jwt.png)

You then need to create a new JSON key.

![New JSON key](/go/api-new_key.png)

Select an expiration date that suits you.

![Key expiration date](/go/api-expiration_date.png)

And make sure to download it, as you won't be able to retrieve it again.

![Key download](/go/api-download_key.png)
...
![Service User Panel](/go/api-service_user_panel.png)

Give the service user a name and a user name. Select `Bearer` as *Access Token Type*.

![Service User Creation](/go/api-create_service_user.png)
...
![Create project role in console](/go/api-project-role.png)
...
![Created authorization in console](/go/api-project-auth.png)
