...
Please also consult our [guide](/docs/manage/org-management/user-management/reg-create-user) on how to create users.
...
<Callout type="info" title="We will improve the bulk import interface for users in the future.">
...
Existing password hashes can be imported if they use a supported [hash algorithm](/docs/learn/architecture/secrets#hashed-secrets).
...
<Callout type="info" title="Verifiers">
...
To configure the default hasher for new user passwords, set the `Algorithm` of the `PasswordHasher` in the [runtime configuration file](/docs/operate/configure/configure#runtime-configuration-file)
...
<Callout type="info" title="Currently it is not possible to migrate passkeys directly from another system.">

</Callout>

## Users linked to an external IDP

A users `sub` is bound to the external [IDP's Client ID](/docs/manage/policies/default-settings#identity-providers).
...
You can use an Action with [post-creation flow](/docs/reference/actions/external-authentication#post-creation) to pull information such as roles from the old system and apply them to the user in ZITADEL.
...
<Callout type="info" title="Metadata must be added to users after the users were created. Currently metadata can't be added during user creation.">
[API reference: User Metadata](/docs/reference/resources/mgmt/user-metadata)

</Callout>

Request metadata from the userinfo endpoint by passing the required [reserved scope](/docs/reference/openidoauth/scopes#reserved-scopes) in your auth request.
With the [complement token flow](/docs/reference/actions/complement-token), you can also transform metadata (or roles) to custom claims.
...
[API reference: User Authorization / Grants](/docs/reference/api-v1/auth)
