...
> In our [Point of Sales example](/docs/manage/projects-apps/projects#example) from the projects guide, you could add a `stripeCustomerId` as a metadata key. In your client application you could then easily fetch the customer from Stripe APIs for your payments.
...
To get your metadata from the userinfo endpoint, add `urn:zitadel:iam:user:metadata` to your authentication request. Take a look at our reserved scopes [here](/docs/reference/openidoauth/scopes#reserved-scopes) or take a look at our [metadata guide](/docs/build-and-integrate/customization/general/user-metadata).
