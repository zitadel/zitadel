...
To migrate, you either use the [Migrate Generic OIDC Identity Provider (Instance)](/docs/reference/api-v1/admin/zitadel.admin.v1.AdminService.MigrateGenericOIDCProvider) or [Migrate Generic OIDC Identity Provider (Organization)](/docs/reference/api-v1/management/zitadel.management.v1.ManagementService.MigrateGenericOIDCProvider) API request.
These calls change the type of the provider and don't delete any linked users. 

<Callout type="info" title="Linked users will not notice the change and be able to login as usual.">

</Callout>

### Google Configuration

The available configuration is described in [Google Configuration](/docs/build-and-integrate/social-login/google).
