...
Please follow [this guide here](/docs/build-and-integrate/token-introspection/private-key-jwt), ignoring the first step as we already have the `.json`-key-file from the serviceaccount.
