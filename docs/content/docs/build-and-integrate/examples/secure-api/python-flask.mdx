...
The API will validate the access token on the [introspect endpoint](/docs/reference/openidoauth/endpoints#introspection-endpoint) and will receive the user's roles from ZITADEL.

The API application uses [Client Secret Basic](/docs/reference/openidoauth/authn-methods#client-secret-basic) to authenticate against ZITADEL and access the introspection endpoint.
You can use any valid access_token from a user or service account to send requests to the example API.
In this example we will use a service account with a [personal access token](/docs/build-and-integrate/m2m/personal-access-token) which can be used directly to access the example API.
...
1. If you don't have an instance yet, please go ahead and create an instance as explained [here](/docs/build-and-integrate/quickstarts/quickstart). Also, create a new project by following the steps [here](/docs/build-and-integrate/quickstarts/quickstart).

2. You must create an API application in your project. Follow [this guide](/docs/manage/projects-apps/applications) to create a new application of type "API" with authentication method "Basic". Save both the ClientID and ClientSecret after you create the application. 
...
1. Create a service user and a Personal Access Token (PAT) for that user by following [this guide](/docs/build-and-integrate/m2m/personal-access-token#create-a-service-user-with-a-pat). 
2. To enable authorization, follow [this guide](/docs/manage/org-management/roles) to create a role `read:messages` on your project. 
3. Next, create an authorization for the service user you created by adding the role `read:messages` to the user. Follow this [guide](/docs/manage/org-management/roles#authorizations) for more information on creating an authorization. 
