...
<Callout type="info" title="This documentation references our [example](https://github.com/zitadel/example-django-python-oidc) on GitHub.">
...
We use [Authorization Code](/docs/references/openidoauth/grant-types#authorization-code) for our Django application.
...
<Callout type="warn" title='Never store and commit secrets in the ".env" or settings.py file'>
...
<Callout type="info" title='Additional permission checks could be done with "permission_required" from "django.contrib.auth.decorators" also described in the [Django documentation](https://docs.djangoproject.com/en/5.0/topics/auth/customizing/#custom-permissions).'>
