...
<Callout type="info" title="This documentation references our [example](https://github.com/zitadel/example-symfony-oidc) on GitHub.">
...
<Callout type="info" title="Skip this step if you are connecting ZITADEL to an existing application.">
...
<Callout type="info" title="The remainder of this guide assumes a Symfony project which already includes all web app bundles, such as security, routing and ORM.">
...
<Callout type="info" title="You can skip this step, if you already have an existing User object in your project.">
...
<Callout type="warn" title="Never store and commit secrets in a `.env` file. Use a `env.local` file instead and make sure the file is in `.gitignore`.">
