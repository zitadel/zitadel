...
For further details and in-depth exploration, you can refer to the [ZITADEL API documentation](/docs/references/introduction).
...
[Reference documentation for authentication API](/docs/references/introduction#authentication)
...
[Reference documentation for system API](/docs/references/introduction#system)
...
This guide covers a specific case of service user authentication when requesting access to the [ZITADEL APIs](/docs/references/introduction).
While PAT can be used directly to access the ZITADEL APIS, the more secure authentication methods private key JWT and client credentials must include the [reserved scope](/docs/references/openidoauth/scopes) `urn:zitadel:iam:org:project:id:zitadel:aud` when requesting an access from the token endpoint.
...
For an example see the [client secret basic authentication method reference](/docs/references/openidoauth/authn-methods#client-secret-basic).
