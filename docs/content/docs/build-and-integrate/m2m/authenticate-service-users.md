...
Service users in combination with [Manager](/docs/learn/structure/managers) permissions are used to access ZITADEL's APIs, for example, to manage user resources.
...
You can [use service users to access ZITADEL APIs](/docs/reference/api/guides/access-zitadel-apis), follow the guides to learn how to access the different ZITADEL APIs.
...
Using the [Event API](/docs/reference/api/guides/event-api) you can use these logs for further analysis or to integrate the logs with [external SOC / SIEM](/docs/operate/operations/observability/external-audit-log) systems.
...
1. **Creation:** Access the ZITADEL management console and create a new service user. Assign a descriptive name that reflects its purpose. Follow our detailed guide on [how to create service users](/docs/manage/org-management/users).
...
* [OIDC JWT with private key](/docs/reference/openidoauth/authn-methods#jwt-with-private-key) authentication method reference
* [Access ZITADEL APIs](/docs/reference/api/guides/access-zitadel-apis)
* Validate access tokens with [token introspection with private key jwt](/docs/build-and-integrate/token-inspection/private-key-jwt)
