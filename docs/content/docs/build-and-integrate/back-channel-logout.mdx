...
<Callout type="info" title="Back-Channel Logout is currently an experimental [beta](/docs/manage-and-govern/support/software-release-cycles-support#beta) feature.">
...
See [OIDC Flow](/docs/build-and-integrate/login/oidc/login-users) for more details.
...
See [OIDC Flow](/docs/build-and-integrate/login/oidc/login-users) again for details.
...
configuration, e.g. [creating an OIDC application](https://zitadel.com/docs/references/resources/mgmt/management-service-add-oidc-app).
...
<Callout type="info" title="Note however, that a Logout Token must never contain a `nonce` claim, to make sure it cannot be used as an ID Token.">
