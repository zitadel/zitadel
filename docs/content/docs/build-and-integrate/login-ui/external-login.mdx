...
If you haven’t added a provider yet, have a look at the following guide first: [Identity Providers](/docs/build-and-integrate/identity-providers/introduction)
...
[Start Identity Provider Intent Documentation](/docs/references/resources/user_service_v2/user-service-start-identity-provider-intent)
...
<Callout type="warn" title="Note that the redirect URL is `https://${CUSTOM_DOMAIN}/idps/callback` when using the new V2 hosted login compared to the V1 hosted login, which was `https://${CUSTOM_DOMAIN}/ui/login/login/externalidp/callback`.">
...
[Retrieve Identity Provider Intent Documentation](/docs/references/resources/user_service_v2/user-service-retrieve-identity-provider-intent)
...
[Create User API Documentation](/docs/references/resources/user_service_v2/user-service-add-human-user)
...
[Add IDP Link to existing user documentation](/docs/references/resources/user_service_v2/user-service-add-idp-link)
