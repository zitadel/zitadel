...
To get the current state of the session, you can call the [GetSession endpoint](/docs/references/api/session/zitadel.session.v2.SessionService.GetSession),
resp. you can get several by [searching sessions](/docs/references/api/session/zitadel.session.v2.SessionService.ListSessions).
...
<Callout type="info" title="Note that ZITADEL will reject any expired sessions automatically and that they cannot be updated anymore.">
...
<Callout type="info" title="Note that if the `lifetime` was not set, the session will never expire.">
