...
In case one of your applications requires the [OAuth2 Device Authorization Grant](/docs/build-and-integrate/login/oidc/device-authorization) this guide will show you how to implement
...
<Callout type="info" title="The version and the optional custom URI for the available login UI is configurable under the application settings.">
...
[Get Device Authorization Request Documentation](/docs/references/api/oidc/zitadel.oidc.v2.OIDCService.GetDeviceAuthorizationRequest)
...
Implement all the steps you like the user the go trough by [creating](/docs/references/api/session/zitadel.session.v2.SessionService.CreateSession) and [updating](/docs/references/api/session/zitadel.session.v2.SessionService.SetSession) the user-session.
...
Read more about the [Authorize or Deny Device Authorization Request Documentation](/docs/references/api/oidc/zitadel.oidc.v2.OIDCService.AuthorizeOrDenyDeviceAuthorization)
...
Additionally, we recommend you to proxy all the other [OIDC relevant endpoints](/docs/references/openidoauth/endpoints).