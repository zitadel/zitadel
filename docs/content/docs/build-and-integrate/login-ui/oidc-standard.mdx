...
[Get Auth Request By ID Documentation](/docs/references/api/oidc/zitadel.oidc.v2.OIDCService.GetAuthRequest)
...
Read more about the [Finalize Auth Request Documentation](/docs/references/api/oidc/zitadel.oidc.v2.OIDCService.CreateCallback)
...
Prompt the user to select a session, terminate it using the [corresponding endpoint](/docs/references/api/session/zitadel.session.v2.SessionService.DeleteSession) and send the user to the `post_logout_redirect` URL.
