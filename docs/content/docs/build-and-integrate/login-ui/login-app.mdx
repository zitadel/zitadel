...
The Login app is then able to load an [AuthRequest](/docs/references/resources/oidc_service_v2/oidc-service-get-auth-request#get-oidc-auth-request-details).
...
If enough user information is retrieved and the user is authenticated according to the policies, the flow is finalized by [requesting a the callback url](/docs/references/resources/oidc_service_v2/oidc-service-create-callback) for the auth request and the user is redirected back to the application.
...
To register your login domain on your instance, [add](/docs/references/resources/admin/admin-service-add-instance-trusted-domain) your domain on your instances trusted domains.
...
Also ensure your login domain is registered on your instance by adding it as a [trusted domain](/docs/references/resources/admin/admin-service-add-instance-trusted-domain).
