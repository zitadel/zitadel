...
The Login app is then able to load an [AuthRequest](/docs/references/api/oidc/zitadel.oidc.v2.OIDCService.GetAuthRequest).
...
If enough user information is retrieved and the user is authenticated according to the policies, the flow is finalized by [requesting a the callback url](/docs/references/api/oidc/zitadel.oidc.v2.OIDCService.CreateCallback) for the auth request and the user is redirected back to the application.
...
To register your login domain on your instance, [add](/docs/references/api-v1/admin/zitadel.admin.v1.AdminService.AddInstanceTrustedDomain) your domain on your instances trusted domains.
...
Also ensure your login domain is registered on your instance by adding it as a [trusted domain](/docs/references/api-v1/admin/zitadel.admin.v1.AdminService.AddInstanceTrustedDomain).
