...
import TokenExchangeTypes from '../references/openidoauth/_token_exchange_types.mdx';
import TokenExchangeRequest from '../references/openidoauth/_token_exchange_request.mdx';
import TokenExchangeResponse from '../references/openidoauth/_token_exchange_response.mdx';
...
<Callout type="info" title="Token Exchange is currently an [experimental beta](/docs/manage-and-govern/support/software-release-cycles-support#beta) feature. Be sure to enable it on the [feature API](#feature-api) before using it.">
...
- [Claims](/docs/references/openidoauth/claims)
- [Scope](/docs/references/openidoauth/scopes)
- Audience
...
At the moment we do not support sending refresh tokens as part of the Token Exchange grant. Instead, use the [`refresh_token` grant](/docs/references/openidoauth/endpoints#refresh-token-grant).
...
[Scope](/docs/references/openidoauth/scopes) is an optional parameter that allows changing the scope of the supplied token, for the requested token. Scope can be entirely different from any of the supplied tokens. It can be used to extend or decrease the scope of the new token.
...
The `refresh_token` may contain a new refresh token that can be used to refresh the `access_token` at a later moment. ZITADEL does not allow using refresh tokens in the Token Exchange grant. Refresh tokens can be used for the [`refresh_token` grant](/docs/references/openidoauth/endpoints#refresh-token-grant) instead, including ones obtained through Token Exchange.
...
If you are self-hosting, you can also enable the feature for the complete system (all instances) using the [set system level features](/docs/references/resources/feature_service_v2/feature-service-set-system-features) endpoint.
...
As indicated by the `token_type` the new access token can be used as Bearer. Once the web-app will make a call to one of the aggregate APIs, that API can make an [introspection](/docs/references/openidoauth/endpoints#introspection_endpoint) call with the access token. Note we use the credentials of the API here:
...
Next we need to configure which users are allowed to impersonate other users. ZITADEL provides 4 [management roles](/docs/manage-and-govern/console/managers):
...
In the access token [claims](/docs/references/openidoauth/claims) we can see that the subject and audience are taken from the `subject_token`. The `act` claim contains the subject and issuer of the `actor_token`, so we can always determine the impersonator that obtained the token.
