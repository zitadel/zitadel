...
import TokenExchangeTypes from '../../../reference/openidoauth/_token_exchange_types.mdx';
import TokenExchangeRequest from '../../../reference/openidoauth/_token_exchange_request.mdx';
import TokenExchangeResponse from '../../../reference/openidoauth/_token_exchange_response.mdx';
...
<Callout type="info" title="Token Exchange is currently an [experimental beta](/docs/manage/support/software-release-cycles-support#beta) feature. Be sure to enable it on the [feature API](#feature-api) before using it.">
...
- Integrate your app with the [OIDC flow](/docs/build-and-integrate/login/oidc/login-users) to obtain tokens
- [Claims](/docs/reference/openidoauth/claims)
- [Scope](/docs/reference/openidoauth/scopes)
- Audience
...
At the moment we do not support sending refresh tokens as part of the Token Exchange grant. Instead, use the [`refresh_token` grant](/docs/reference/openidoauth/endpoints#refresh-token-grant).
...
[Scope](/docs/reference/openidoauth/scopes) is an optional parameter that allows changing the scope of the supplied token, for the requested token. Scope can be entirely different from any of the supplied tokens. It can be used to extend or decrease the scope of the new token.
...
The `refresh_token` may contain a new refresh token that can be used to refresh the `access_token` at a later moment. ZITADEL does not allow using refresh tokens in the Token Exchange grant. Refresh tokens can be used for the [`refresh_token` grant](/docs/reference/openidoauth/endpoints#refresh-token-grant) instead, including ones obtained through Token Exchange.
...
If you are self-hosting, you can also enable the feature for the complete system (all instances) using the [set system level features](/docs/reference/api/feature/zitadel.feature.v2.FeatureService.SetSystemFeatures) endpoint.
...
As indicated by the `token_type` the new access token can be used as Bearer. Once the web-app will make a call to one of the aggregate APIs, that API can make an [introspection](/docs/reference/openidoauth/endpoints#introspection-endpoint) call with the access token. Note we use the credentials of the API here:
...
<Callout type="info" title="You can also obtain an ID token in the `id_token` response field by requestion an access token and the `openid` scope.">
...
<Callout type="info" title="Currently impersonated tokens cannot be used for the ZITADEL API. This is to prevent privilege escalation where a impersonator could become an IAM owner, for example. We might enable the use of impersonated tokens in the future.">
...
Next we need to configure which users are allowed to impersonate other users. ZITADEL provides 4 [management roles](/docs/manage/org-management/managers):
...
In the access token [claims](/docs/reference/openidoauth/claims) we can see that the subject and audience are taken from the `subject_token`. The `act` claim contains the subject and issuer of the `actor_token`, so we can always determine the impersonator that obtained the token.
...
<Callout type="info" title="User ID as subject token is an experimental addition and is provided pending evaluation of our community. This method might be considered insecure and trust is fully placed into the app making the request. This might be removed in the future.">
...
In the [instance event list](/docs/learn/event-sourcing/overview) the `User impersonated` carries the actor in the payload:
