...
Authenticating users through the OpenID Connect protocol typically requires an application to redirect the user with an [Auth Request](https://zitadel.com/playgrounds/oidc) to the identity provider that contains information such as the requesting application, [scopes](/docs/references/openidoauth/scopes), and redirect url.
The identity provider is not part of the original application, but a standalone service like ZITADEL that may run under the [same domain](/docs/concepts/features/custom-domain)
...
ZITADEL's [Session API](/docs/references/api/session/zitadel.session.v2.SessionService.CreateSession) provides developers with a straightforward method to manage user sessions within their applications.
The Session API is not an industry-standard and can be used instead of OpenID Connect or SAML to authenticate users by [building your own custom login user interface](/docs/build-and-integrate/login-ui/login-app).
...
We have dedicated guides on [how to build your custom login UI](/docs/build-and-integrate/login-ui/login-app) with ZITADEL.
...
2. **Customize business process**: You might want to change existing flows that are provided by the hosted login to fit your business process. Make sure to validate that your customization can't be handled by [Actions](/docs/manage-and-govern/customize/behavior) instead.
3. **Customize branding**: We designed the hosted login with security in mind, which limits the [customization capabilities](/docs/manage-and-govern/customize/branding). Besides that we might not be able to handle all requirements for custom branding. If you believe your customization should be part of the hosted login instead, please open an [improvement idea](https://github.com/zitadel/zitadel/issues/new/choose).
...
- Learn how to [register and onboard users](/docs/build-and-integrate/onboarding/end-users)
- Learn how to [build your own login UI](/docs/build-and-integrate/login-ui/login-app) based on ZITADEL
- Learn how to [configure ZITADEL](/docs/manage-and-govern/console/overview) through the Console
