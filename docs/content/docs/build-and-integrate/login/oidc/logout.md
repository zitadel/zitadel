...
With the RP initiated flow all logout processes are triggered by a request from the client (e.g your application) through a well defined standard API by redirecting the user-agent to the [end_session_endpoint](/docs/references/openidoauth/endpoints#end_session_endpoint).
...
<Callout type="info" title="This is not yet implemented in ZITADEL">
