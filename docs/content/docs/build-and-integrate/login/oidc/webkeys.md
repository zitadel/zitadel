...
JWT access tokens, instead of [introspection](/docs/reference/openidoauth/endpoints#introspection-endpoint).
...
ZITADEL provides a resource based [web keys API](/docs/reference/api/webkey/zitadel.webkey.v2.WebKeyService.ListWebKeys).
...
- Until access and ID tokens are expired. See [OIDC token lifetimes](/docs/manage/policies/default-settings#oidc-token-lifetimes-and-expiration).
