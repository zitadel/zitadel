...
Login methods can be a user's username, containing the username and an [organization domain](/docs/manage-and-govern/console/organizations#domain-verification-and-primary-domain), their email addresses, or their phone numbers.
...
The hosted login page supports various authentication methods, including traditional username/password authentication, social login options, multi-factor authentication (MFA), and passwordless authentication methods like [passkeys](/docs/concepts/features/passkeys).
...
ZITADEL offers various [identity provider templates](/docs/build-and-integrate/identity-providers/introduction) to integrate providers such as [Okta](/docs/build-and-integrate/identity-providers/okta-oidc), [Entra ID](/docs/build-and-integrate/identity-providers/azure-ad-oidc) or on-premise [LDAP](/docs/build-and-integrate/identity-providers/ldap).
...
2. **Custom Authentication Configurations**: Allows tailored [authentication settings](/docs/manage-and-govern/console/default-settings#login-behavior-and-access), [branding](/docs/manage-and-govern/customize/branding), and policies for each tenant.
3. **Centralized Management**: Provides [centralized administration](/docs/manage-and-govern/console/managers) for efficient management across all tenants.
4. **Scalability and Flexibility**: Scales seamlessly to accommodate growing organizations of all sizes.
5. **Domain Discovery**: Starting on a central login page, route users to their tenant based on their email address or other user attributes. Authentication settings will be applied automatically based on the organization's policies, this includes routing users seamlessly to third party identity providers like [Entra ID](/docs/build-and-integrate/identity-providers/azure-ad-oidc).

### Customization options

While the hosted login page provides a default authentication interface out-of-the-box, ZITADEL offers [customization options](/docs/manage-and-govern/customize/branding) to tailor the login page to match your application's branding and user experience requirements.
...
By default, the displayed branding is defined [based on the user's domain](/docs/build-and-integrate/solution-scenarios/domain-discovery). In case you want to show the branding of a specific organization by default, you need to either pass a primary domain scope (`urn:zitadel:iam:org:domain:primary:{domainname}`) with the authorization request, or define the behavior on your Project's settings.
...
- Instance or organization [login policy](/docs/manage-and-govern/console/default-settings#login-behavior-and-access) is set
...
You can disable the prompt, in case multifactor authentication is not enforced by setting the [**Multifactor Init Lifetime**](/docs/manage-and-govern/console/default-settings#login-lifetimes) to 0.
...
The passwordless with [passkeys](/docs/concepts/features/passkeys) login flow follows the FIDO2 / WebAuthN standard.
...
<Tabs queryString="deployment">
    ...
    <Tab value="vercel_custom" label="Vercel Custom Login Deployment">
        ...
        1. [Create a service user](https://zitadel.com/docs/build-and-integrate/service-users/personal-access-token#create-a-service-user-with-a-pat) with a PAT in your instance
        2. Give the user IAM_LOGIN_CLIENT Permissions in the default settings (CUSTOM_DOMAIN/ui/console/instance?id=organizations)
        Note: [Zitadel Manager Guide](https://zitadel.com/docs/manage-and-govern/console/managers)
        ...
        5. Add the domain where your login UI is hosted to the [trusted domains](https://zitadel.com/docs/references/resources/admin/admin-service-add-instance-trusted-domain) in Zitadel. (Example: my-new-zitadel-login.vercel.app)
