...
It employs best practices for securing authentication processes, such as encryption, token-based authentication, and adherence to protocols like OAuth 2.0, [OpenID Connect](/docs/build-and-integrate/login/oidc/login-users), and [SAML](/docs/build-and-integrate/login/saml).
...
Login methods can be a user's username, containing the username and an [organization domain](/docs/manage/org-management/organizations#domain-verification-and-primary-domain), their email addresses, or their phone numbers.
By default, all of these login methods are allowed and can be adjusted by [Managers](/docs/learn/structure/managers) to meet their requirements.

### Support for multiple authentication methods

The hosted login page supports various authentication methods, including traditional username/password authentication, social login options, multi-factor authentication (MFA), and passwordless authentication methods like [passkeys](/docs/learn/features/passkeys).
...
ZITADEL acts as an [identity broker](/docs/learn/features/identity-brokering) between your applications and different external identity providers, reducing the implementation effort for developers.
...
ZITADEL offers various [identity provider templates](/docs/manage/federation/introduction) to integrate providers such as [Okta](/docs/manage/federation/okta-oidc), [Entra ID](/docs/manage/federation/azure-ad-oidc) or on-premise [LDAP](/docs/manage/federation/ldap).

### Multi-tenancy authentication

ZITADEL simplifies multi-tenancy authentication by securely managing authentication for multiple tenants, called [Organizations](/docs/learn/structure/organizations), within a single [instance](/docs/learn/structure/instance).

Key features include:

1. **Secure Tenant Isolation**: Ensures robust security measures to prevent unauthorized access between tenants, maintaining data privacy and compliance. [Managers](/docs/learn/structure/managers) for an organization have only access to data and configuration within their Organization.
2. **Custom Authentication Configurations**: Allows tailored [authentication settings](/docs/manage/policies/default-settings#login-behavior-and-access), [branding](/docs/build-and-integrate/customization/general/branding), and policies for each tenant.
3. **Centralized Management**: Provides [centralized administration](/docs/manage/org-management/managers) for efficient management across all tenants.
4. **Scalability and Flexibility**: Scales seamlessly to accommodate growing organizations of all sizes.
5. **Domain Discovery**: Starting on a central login page, route users to their tenant based on their email address or other user attributes. Authentication settings will be applied automatically based on the organization's policies, this includes routing users seamlessly to third party identity providers like [Entra ID](/docs/manage/federation/azure-ad-oidc).

### Customization options

While the hosted login page provides a default authentication interface out-of-the-box, ZITADEL offers [customization options](/docs/build-and-integrate/customization/general/branding) to tailor the login page to match your application's branding and user experience requirements.
...
<Callout type="info" title="Customization and Branding">
...
By default, the displayed branding is defined [based on the user's domain](/docs/build-and-integrate/patterns/domain-discovery). In case you want to show the branding of a specific organization by default, you need to either pass a primary domain scope (`urn:zitadel:iam:org:domain:primary:{domainname}`) with the authorization request, or define the behavior on your Project's settings.
...
<Callout type="info" title="This behavior can be changed with the authorization request. Please refer to our [guide](/docs/build-and-integrate/login/oidc/login-users).">

</Callout>

### Self-service for users

ZITADEL's hosted login page offers [many self-service flows](/docs/learn/features/selfservice) that allow users to set up authentication methods or recover their login information.
...
- Instance or organization [login policy](/docs/manage/policies/default-settings#login-behavior-and-access) is set
...
<Callout type="info" title="Disabling multifactor prompt">
You can disable the prompt, in case multifactor authentication is not enforced by setting the [**Multifactor Init Lifetime**](/docs/manage/policies/default-settings#login-lifetimes) to 0.
...
The passwordless with [passkeys](/docs/learn/features/passkeys) login flow follows the FIDO2 / WebAuthN standard.
...
We have worked on a new, self-hostable implementation of our hosted MIT licensed login built with Next.js and leveraging our [Session API](/docs/build-and-integrate/login/login-users#session-api).
...
<Tabs queryString="deployment">
    <Tab value="zitadel_cloud" label="Activate Zitadel Cloud V2 Login" default="true">
    ...
    <Tab value="vercel_custom" label="Vercel Custom Login Deployment">
        ...
        1. [Create a service user](https://zitadel.com/docs/build-and-integrate/m2m/personal-access-token#create-a-service-user-with-a-pat) with a PAT in your instance
        2. Give the user IAM_LOGIN_CLIENT Permissions in the default settings (CUSTOM_DOMAIN/ui/console/instance?id=organizations)
        Note: [Zitadel Manager Guide](https://zitadel.com/docs/manage/org-management/managers)
        ...
        5. Add the domain where your login UI is hosted to the [trusted domains](https://zitadel.com/docs/reference/resources/admin/admin-service-add-instance-trusted-domain) in Zitadel. (Example: my-new-zitadel-login.vercel.app)
