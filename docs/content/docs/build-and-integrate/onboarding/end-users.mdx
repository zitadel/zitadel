...
OpenID Connect (OIDC) allows you to control the initial screen by sending a [prompt](/docs/references/openidoauth/endpoints#additional-parameters) parameter in the authorization request.
...
<Callout type="info" title="The options are dependent on the settings of the organization.">
...
Please follow the configuration guides for the needed providers: [Let Users Login with Preferred Identity Provider in ZITADEL](/docs/build-and-integrate/identity-providers/introduction)
...
You can find all the guides here: [Build your own login UI](/docs/build-and-integrate/login-ui/login-app)

#### Custom fields

The [create user request](/docs/references/api/user/zitadel.user.v2.UserService.AddHumanUser) also allows you to add [metadata](/docs/manage-and-govern/customize/user-metadata) (key, value) to the user.
...
Those metadata can also directly be included in the [token](/docs/manage-and-govern/customize/user-metadata#use-tokens-to-get-user-metadata) of the user.
