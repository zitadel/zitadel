...
In the example there is a service provider with a ZITADEL instance running on a [custom domain](/docs/manage-and-govern/cloud/instances#add-custom-domain) on `login.mycompany.com`.
...
In case of Alpha users will login via an external identity provider (eg, [Entra ID](/docs/build-and-integrate/identity-providers/azure-ad-oidc)).
...
This means that you have to configure the [Login and Access](/docs/manage-and-govern/console/default-settings#login-behavior-and-access) Policy and [Identity Providers](/docs/manage-and-govern/console/default-settings#identity-providers) for the **CIAM** users on the instance itself.
...
Set **CIAM** as [default organization](/docs/manage-and-govern/console/organizations#default-organization).
...
In the [Login Behavior and Security Settings](/docs/manage-and-govern/console/default-settings#login-behavior-and-access) enable "Domain discovery allowed"

### Configure login with email

Follow this [configuration guide](/docs/build-and-integrate/solution-scenarios/configurations#use-email-to-login) to allow users to login with their email address.

### Other considerations

You can also have multiple custom domains pointing to the same instance as described in this [configuration guide](/docs/build-and-integrate/solution-scenarios/configurations#custom-application-domain-per-organization). In our example you could also use `alpha.mycompany.com` to show the login page of your instance.

The domain of your email notification can be changed by [setting up your SMTP](/docs/manage-and-govern/console/default-settings#smtp).
...
Now you can configure an [external identity provider](/docs/manage-and-govern/console/default-settings#identity-providers).
...
Make sure to [Force MFA](/docs/manage-and-govern/console/default-settings#multifactor-mfa) so that users must setup a second factor for authentication.

### Verify domains

Switch to the organization **Alpha** and navigate to the settings and "Verified domains".
Verify the domain alpha.com following the [organization guide](/docs/manage-and-govern/console/organizations#domain-verification-and-primary-domain).

Do the same for the **Beta** organization.

<Callout type="info" title="You can also disable domain verification with acme challenge in the [default settings](/docs/manage-and-govern/console/default-settings#domain-settings).">
