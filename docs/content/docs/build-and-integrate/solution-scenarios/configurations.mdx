...
2. Configure the needed identity provider: Read this [guide](../identity-providers/introduction) if you don't know how
...
More about the [scopes](/docs/references/openidoauth/scopes#reserved-scopes)
...
Read more about [applications](/docs/manage-and-govern/console/applications) and the [redirect urls](/docs/manage-and-govern/console/applications)
...
<Callout type="info" title="Please note, that SameSite None requires the cookie to be flagged `secure`, which means it must be sent over TLS (HTTPS) or localhost.">
