...
The [OpenID Connect & OAuth endpoints](/docs/references/openidoauth/endpoints) and [SAML 2.0 endpoints](/docs/references/saml/endpoints) are implemented and exposed according to the specific standards. Managing resources such as users, organizations, instances, or settings must be done with the different [ZITADEL APIs](#zitadel-apis-resource-based).

[Actions](/docs/references/actions/introduction) allow to extend ZITADEL with custom code to change default behaviors or calling external systems.
...
Please refer to our guides how to [authenticate users](/docs/build-and-integrate/login/login-users) through an interactive authentication process.

For user authentication on devices with limited accessibility (eg, SmartTV, Smart Watch etc.) use the [device authorization grant](/docs/build-and-integrate/login/oidc/device-authorization).

Additionally, you can use the [session API](/docs/references/api/session/zitadel.session.v2.SessionService.CreateSession) to authenticate users, for example by building a [custom login UI](/docs/build-and-integrate/login-ui/login-app).

### Authenticate service users and machines

Service users allow for machine-to-machine (M2M) communication.
Follow the guides to learn how to [authenticate service users](/docs/build-and-integrate/service-users/authenticate-service-users).

Accessing the ZITADEL APIs through a service user might require additional steps, please follow the guide on how to [access ZITADEL APIs](/docs/build-and-integrate/zitadel-apis/access-zitadel-apis) to include the correct audience scope in your requests.

### OpenID Connect & OAuth

- [OpenID Connect endpoints](/docs/references/openidoauth/endpoints) definition
- Standard and reserved [scopes reference](/docs/references/openidoauth/scopes)
- Standard, custom, and reserved [claims reference](/docs/references/openidoauth/claims)
...
- [SAML 2.0 endpoints](/docs/references/saml/endpoints) definition
...
ZITADEL allows to authenticate users by creating a session with the [Session API](/docs/references/api/session/zitadel.session.v2.SessionService.CreateSession), get OIDC authentication request details with the [OIDC service API](/docs/references/api/oidc/zitadel.oidc.v2.OIDCService.GetAuthRequest) or get SAML request details with the [SAML service API](/docs/references/api/saml/zitadel.saml.v2.SAMLService.GetSAMLRequest).
User authorizations can be [retrieved as roles from our APIs](/docs/build-and-integrate/retrieve-user-roles).

Refer to our guide to learn how to [build your own login UI](/docs/build-and-integrate/login-ui/login-app)

## ZITADEL APIs (resource-based)

ZITADEL provides APIs for each [core resource](/docs/references/v2):

- [User](/docs/references/api/user/zitadel.user.v2.UserService.ListUsers)
- [Session](/docs/references/api/session/zitadel.session.v2.SessionService.CreateSession)
- [Settings](/docs/references/api/settings/zitadel.settings.v2.SettingsService.GetGeneralSettings)

<Callout type="info" title="We are migrating to a resource-based API approach.">
...
<Callout type="info" title="Prefer resource-based API">
...
API Reference:
[OpenAPI Docs](/docs/references/api-v1/auth/zitadel.auth.v1.AuthService.GetMyUser)
...
API Reference:
[OpenAPI Docs](/docs/references/api-v1/management/zitadel.management.v1.ManagementService.GetMyOrg)
...
API Reference:
[OpenAPI Docs](/docs/references/api-v1/admin/zitadel.admin.v1.AdminService.GetMyInstance)
...
Checkout the guide how to [access the ZITADEL System API](/docs/build-and-integrate/zitadel-apis/access-zitadel-system-api).
...
API Reference:
[OpenAPI Docs](/docs/references/api-v1/system/zitadel.system.v1.SystemService.ListInstances)
...
ZITADEL provides some [official and community supported SDKs](/docs/build-and-integrate/sdk-examples/introduction) for multiple languages and frameworks.
...
Are you self-hosting and having troubles with _Instance not found_ errors? [Check out this page](/docs/operate-and-self-host/manage/custom-domain).
