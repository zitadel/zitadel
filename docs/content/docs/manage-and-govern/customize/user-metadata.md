...
You can do so by using [Console](../console/users) or [setting user metadata](/docs/references/resources/mgmt/management-service-set-user-metadata) through the management API.
...
You can request the user's metadata with the [List My Metadata](/docs/references/resources/auth/auth-service-list-my-metadata) method:
...
Learn everything about how to build your own registration UI [here](/docs/build-and-integrate/onboarding/end-users#build-your-own-registration-form).
...
The user that calls the management service must have [manager permissions](/docs/manage-and-govern/console/managers).
A user can be either a human user or a service user.

You can get [metadata of a user filtered by your query](/docs/references/resources/mgmt/management-service-list-user-metadata) or [get a metadata object from a user by a specific key](/docs/references/resources/mgmt/management-service-get-user-metadata).
The management service allows you to set and delete metadata, see the [API documentation for users](/docs/references/resources/mgmt/users).
