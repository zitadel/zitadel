...
<Tabs>
    <Tab value="human" label="Human User" default>
        ...
        - **Send an invitation E-Mail for authentication setup and E-Mail verification**: The user will receive an email and be able to setup an authentication method (e.g Password, Passkey, External SSO).
            ...
            - When using the [Zitadel Login V2](/docs/build-and-integrate/login/hosted-login#hosted-login-version-2-beta) the user has the option to choose the authentication method (password, passkey, identity provider), based on the configuration of the organization
