...
To configure the default hasher for new user passwords, set the `Algorithm` of the `PasswordHasher` in the [runtime configuration file](/docs/operate-and-self-host/manage/configure/configure#runtime-configuration-file)
...
A users `sub` is bound to the external [IDP's Client ID](/docs/manage-and-govern/console/default-settings#identity-providers).
...
You can use an Action with [post-creation flow](/docs/references/actions/external-authentication#post-creation) to pull information such as roles from the old system and apply them to the user in ZITADEL.
...
[API reference: User Metadata](/docs/references/resources/mgmt/user-metadata)

</Callout>

Request metadata from the userinfo endpoint by passing the required [reserved scope](/docs/references/openidoauth/scopes#reserved-scopes) in your auth request.
With the [complement token flow](/docs/references/actions/complement-token), you can also transform metadata (or roles) to custom claims.
