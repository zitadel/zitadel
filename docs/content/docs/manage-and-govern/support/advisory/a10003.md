...
<Callout type="info" title="If the registration (and also authentication) needs to occur on a specified organization, apps can already">
specify this by providing [an organization scope](/docs/references/openidoauth/scopes#reserved-scopes).
...
Already existing instances will still use the instance settings by default and can switch to the new default by ["Activating the 'LoginDefaultOrg' feature"](/docs/references/api-v1/admin/zitadel.admin.v1.AdminService.ActivateFeatureLoginDefaultOrg) through the Admin API.
**This change is irreversible!**

<Callout type="info" title="Regardless of the change:">
