...
Users with the role IAM_OWNER can change the restrictions of their instance using the [Feature Restrictions Admin API](/docs/reference/api-v1/admin/zitadel.admin.v1.AdminService.GetRestrictions).
...
However, if you are self-hosting and need to control your virtual instances usage, [read about the APIs for limits and quotas](/docs/self-hosting/manage/usage_control) that are intended to be used by system users.
