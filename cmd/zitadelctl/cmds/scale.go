...
func scaleForFunction(
	...
	noZitadel := false
	if gitops {
		noZitadelT, err := crtlgitops.ScaleDown(monitor, gitClient, k8sClient, version, gitops, dbClient)
		if err != nil {
			return err
		}
		noZitadel = noZitadelT
	} else {
		noZitadelT, err := zitadel.ScaleDown(monitor, k8sClient, version, dbClient)
		if err != nil {
			return err
		}
		noZitadel = noZitadelT
	...
	if !noZitadel {
		if gitops {
			if err := crtlgitops.ScaleUp(monitor, gitClient, k8sClient, version, gitops, dbClient); err != nil {
				return err
			}
		} else {
			if err := zitadel.ScaleUp(monitor, k8sClient, version, dbClient); err != nil {
				return err
			}
		}
	}
