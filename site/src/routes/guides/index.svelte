<script context="module">
	export async function preload() {
		const guides = await this.fetch(`guides.json`).then(r => r.json());
		return { guides };
	}
</script>

<script>
	const description = "Frequently Asked Questions about ZITADEL";
...
<div class='faqs stretch'>
	<h1>Quickstarts and Guides</h1>
	{#each guides as faq}

		<article class='faq'>
			<h2>
			<span id={faq.fragment} class="offset-anchor"></span>
			<a class="anchor" sapper:prefetch href='faq#{faq.fragment}' title='{faq.title}'>&nbsp;</a>
			{faq.metadata.title}
			</h2>
			<p>{@html faq.answer}</p>
		</article>
