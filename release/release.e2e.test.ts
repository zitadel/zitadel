...
const ghTokenEnvVar = "ZITADEL_RELEASE_E2E_GH_TOKEN";
...
const REQUIRED_ENV_VARS = [testRepoEnvVar, testBranchEnvVar, ghTokenEnvVar];
...
describe.skipIf(!shouldRunE2E)('Release E2E (Real Fork)', () => {
  ...
  beforeAll(async () => {
    ...
    const localBranch = runCommand('git rev-parse --abbrev-ref HEAD', E2E_TEST_DIR).stdout;
    runInTestRepo(`git merge ${localBranch}`, { ignoreError: true });
