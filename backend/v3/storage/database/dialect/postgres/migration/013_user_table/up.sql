...
CREATE TABLE zitadel.users(
    ...
    , username_organization_unique BOOLEAN DEFAULT FALSE NOT NULL -- this field MUST be filled if the username must be unique on organization level
...
-- user
CREATE UNIQUE INDEX ON zitadel.users(instance_id, organization_id, username) WHERE username_organization_unique IS TRUE;
CREATE UNIQUE INDEX ON zitadel.users(instance_id, username) WHERE username_organization_unique IS FALSE;
