...
var queryUserStmt = "SELECT users.instance_id, users.organization_id, users.id, users.username" +
	...
	`, 'recoveryCodes', CASE WHEN users.recovery_codes IS NOT NULL THEN jsonb_build_object('codes', users.recovery_codes, 'verifiedAt', users.recovery_code_verified_at, 'lastSuccessfullyCheckedAt', users.recovery_code_last_successful_check, 'failedAttempts', users.recovery_code_failed_attempts) ELSE NULL END` +
