...
func (r *CommandSide) addUserGrant(ctx context.Context, userGrant *domain.UserGrant, resourceOwner string) (_ *usergrant.Aggregate, _ *UserGrantWriteModel, err error) {
	...
	exists, err = r.checkProjectExists(ctx, userGrant.ProjectID, resourceOwner)
	if err != nil {
		return nil, nil, err
	}
	if !exists {
		return nil, nil, caos_errs.ThrowPreconditionFailed(nil, "COMMAND-4M0fs", "Errors.Project.NotFound")
	}
