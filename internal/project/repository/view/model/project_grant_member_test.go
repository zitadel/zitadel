...
func TestGrantedProjectMemberAppendEvent(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				event:  &es_models.Event{AggregateID: "AggregateID", Seq: 1, Typ: es_models.EventType(project.GrantMemberAddedType), ResourceOwner: "OrgID", Data: mockProjectGrantMemberData(&es_model.ProjectGrantMember{GrantID: "ProjectGrantID", UserID: "UserID", Roles: []string{"Role"}})},
				member: &ProjectGrantMemberView{},
			},
		...
		},
		{
			...
			args: args{
				event:  &es_models.Event{AggregateID: "AggregateID", Seq: 1, Typ: es_models.EventType(project.GrantMemberAddedType), ResourceOwner: "OrgID", Data: mockProjectGrantMemberData(&es_model.ProjectGrantMember{GrantID: "ProjectGrantID", Roles: []string{"RoleChanged"}})},
