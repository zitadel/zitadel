...
func TestProjectMemberAppendEvent(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				event:  &es_models.Event{AggregateID: "AggregateID", Seq: 1, Typ: es_models.EventType(project.MemberAddedType), ResourceOwner: "OrgID", Data: mockProjectMemberData(&es_model.ProjectMember{UserID: "UserID", Roles: []string{"Role"}})},
				member: &ProjectMemberView{},
			},
		...
		},
		{
			...
			args: args{
				event:  &es_models.Event{AggregateID: "AggregateID", Seq: 1, Typ: es_models.EventType(project.MemberAddedType), ResourceOwner: "OrgID", Data: mockProjectMemberData(&es_model.ProjectMember{UserID: "UserID", Roles: []string{"RoleChanged"}})},
