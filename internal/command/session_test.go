...
func TestCheckTOTP(t *testing.T) {
	ctx := authz.NewMockContext("instance1", "org1", "user1", language.English)
		...
		},
		{
			...
			fields{
				eventstore: expectEventstore(
					...
					expectPush(
						session.NewTerminateEvent(authz.NewMockContext("instance1", "org1", "user1", language.English), &session.NewAggregate("sessionID", "instance1").Aggregate),
					),
				),
			},
			args{
		...
		},
		{
			...
			fields{
				eventstore: expectEventstore(
					...
					expectPush(
						session.NewTerminateEvent(authz.NewMockContext("instance1", "org1", "admin1", language.English), &session.NewAggregate("sessionID", "instance1").Aggregate),
					),
				),
				checkPermission: newMockPermissionCheckAllowed(),
			},
			args{
				ctx:          authz.NewMockContext("instance1", "org1", "admin1", language.English),
				sessionID:    "sessionID",
				sessionToken: "",
			},
			res{
