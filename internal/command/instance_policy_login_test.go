...
func newDefaultLoginPolicyChangedEvent(ctx context.Context, allowRegister, allowUsernamePassword, allowExternalIDP, forceMFA, forceMFALocalOnly,
	...
	redirectURI string,
	passwordLifetime, externalLoginLifetime, mfaInitSkipLifetime, secondFactorLifetime, multiFactorLifetime time.Duration, useDefaultUriForNotificationLinks bool) *instance.LoginPolicyChangedEvent {
	event, _ := instance.NewLoginPolicyChangedEvent(ctx,
		...
		[]policy.LoginPolicyChanges{
			...
			policy.ChangeUseDefaultUriForNotificationLinks(useDefaultUriForNotificationLinks),
		},
	)
	return event
}
