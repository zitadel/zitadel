...
func TestCommandSide_AddOrg(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "add org (remove invalid characters), no error",
			fields: fields{
				eventstore: eventstoreExpect(
					t,
					expectFilterOrgDomainNotFound(),
					...
					expectPush(
						[]*repository.Event{
							eventFromEventPusher(org.NewOrgAddedEvent(context.Background(),
								&org.NewAggregate("org2").Aggregate,
								"Org (with characters like ñ)",
							)),
							eventFromEventPusher(org.NewDomainAddedEvent(context.Background(),
								&org.NewAggregate("org2").Aggregate, "org-with-characters-like.iam-domain",
							)),
							eventFromEventPusher(org.NewDomainVerifiedEvent(context.Background(),
								&org.NewAggregate("org2").Aggregate,
								"org-with-characters-like.iam-domain",
							)),
							eventFromEventPusher(org.NewDomainPrimarySetEvent(context.Background(),
								&org.NewAggregate("org2").Aggregate,
								"org-with-characters-like.iam-domain",
						...
						uniqueConstraintsFromEventConstraint(org.NewAddOrgNameUniqueConstraint("Org (with characters like ñ)")),
						uniqueConstraintsFromEventConstraint(org.NewAddOrgDomainUniqueConstraint("org-with-characters-like.iam-domain")),
						uniqueConstraintsFromEventConstraint(member.NewAddMemberUniqueConstraint("org2", "user1")),
					),
				),
				idGenerator: id_mock.NewIDGeneratorExpectIDs(t, "org2"),
			...
			args: args{
				ctx:           authz.WithRequestedDomain(context.Background(), "iam-domain"),
				name:          "Org (with characters like ñ)",
				userID:        "user1",
				resourceOwner: "org1",
			},
			res: res{
				want: &domain.Org{
					...
					Name:          "Org (with characters like ñ)",
					State:         domain.OrgStateActive,
					PrimaryDomain: "org-with-characters-like.iam-domain",
				},
			},
		},
		{
			name: "add org (extremely long org name), no error",
			fields: fields{
				eventstore: eventstoreExpect(
					t,
					expectFilterOrgDomainNotFound(),
					...
					expectPush(
						[]*repository.Event{
							eventFromEventPusher(org.NewOrgAddedEvent(context.Background(),
								&org.NewAggregate("org2").Aggregate,
								"A long name for an organization that should be truncated for a valid domain",
							)),
							eventFromEventPusher(org.NewDomainAddedEvent(context.Background(),
								&org.NewAggregate("org2").Aggregate, "a-long-name-for-an-organization-that-should-be-truncated-for-a.iam-domain",
							)),
							eventFromEventPusher(org.NewDomainVerifiedEvent(context.Background(),
								&org.NewAggregate("org2").Aggregate,
								"a-long-name-for-an-organization-that-should-be-truncated-for-a.iam-domain",
							)),
							eventFromEventPusher(org.NewDomainPrimarySetEvent(context.Background(),
								&org.NewAggregate("org2").Aggregate,
								"a-long-name-for-an-organization-that-should-be-truncated-for-a.iam-domain",
						...
						uniqueConstraintsFromEventConstraint(org.NewAddOrgNameUniqueConstraint("A long name for an organization that should be truncated for a valid domain")),
						uniqueConstraintsFromEventConstraint(org.NewAddOrgDomainUniqueConstraint("a-long-name-for-an-organization-that-should-be-truncated-for-a.iam-domain")),
						uniqueConstraintsFromEventConstraint(member.NewAddMemberUniqueConstraint("org2", "user1")),
					),
				),
				idGenerator: id_mock.NewIDGeneratorExpectIDs(t, "org2"),
			...
			args: args{
				ctx:           authz.WithRequestedDomain(context.Background(), "iam-domain"),
				name:          "A long name for an organization that should be truncated for a valid domain",
				userID:        "user1",
				resourceOwner: "org1",
			},
			res: res{
				want: &domain.Org{
					...
					Name:          "A long name for an organization that should be truncated for a valid domain",
					State:         domain.OrgStateActive,
					PrimaryDomain: "a-long-name-for-an-organization-that-should-be-truncated-for-a.iam-domain",
				},
			},
		},
	}
