			...
			fields: fields{
				eventstore: eventstoreExpect(
					...
					expectPush(
						[]*repository.Event{
							eventFromEventPusher(
								newLoginPolicyChangedEvent(context.Background(), "org1", false, false, false, false, false, false, domain.PasswordlessTypeNotAllowed, "https://example.com/redirect"),
							),
						},
					),
				),
...
func newLoginPolicyChangedEvent(ctx context.Context, orgID string, usernamePassword, register, externalIDP, mfa, passwordReset, ignoreUnknownUsernames bool, passwordlessType domain.PasswordlessType, redirectURI string) *org.LoginPolicyChangedEvent {
