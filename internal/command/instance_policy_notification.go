	...
	return func() (preparation.CreateCommands, error) {
		return func(ctx context.Context, filter preparation.FilterToQueryReducer) ([]eventstore.Command, error) {
			...
			if writeModel.State == domain.PolicyStateUnspecified || writeModel.State == domain.PolicyStateRemoved {
				return nil, zerrors.ThrowNotFound(nil, "INSTANCE-x891na", "Instance.NotificationPolicy.NotFound")
			}
			change, hasChanged := writeModel.NewChangedEvent(ctx, &a.Aggregate, passwordChange)
			if !hasChanged {
				return nil, zerrors.ThrowPreconditionFailed(nil, "INSTANCE-29x02n", "Instance.NotificationPolicy.NotChanged")
			}
			return []eventstore.Command{
				change,
			}, nil
