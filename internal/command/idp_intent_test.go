...
func TestCommands_AuthFromProvider(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			res{
				content:  "http://localhost:8000/sso?SAMLRequest=nFLNjtMwEH4Va%2B5pTH42qbWJVLZCVFqg2hYO3KbOLLXk2MUzAfbtUdNFKpce9mp%2FvzNzzzj6k1lNcgxP9HMiFvVn9IHN%2BaODKQUTkR2bgCOxEWt2q0%2BPplhog8yUxMUAV5TTbc4pRYk2elCbdQduyJp3bdseqmZY0rKpqapKW1bVoVw%2B39UHW9fF0CCVugb1jRK7GDooFhrUhnmiTWDBIB0Uuigz3Wal3uvGlNpU1aK6%2Bw5qTSwuoMzEo8jJ5LmPFv0xsphWa50zR1Crf10eYuBppLSj9MtZ%2Bvr02EF%2BLpajZVDb1%2FzvXRhc%2BHG77OECYvNxv99m2y%2B7PfTzwM2cPqkPMY0ot0XOL27InmeooSBOXqC%2FRBpJcEDB%2B%2FxKtX%2Fd6WccabPeRu%2FsyxucJGFgR0FArbyPvx8SoVAHkiaCvL9Y%2Fn85%2Fd8AAAD%2F%2Fw%3D%3D&RelayState=state",
				redirect: true,
			},
		},
		{
...
/*
Commented as the requestID is generated by the crewjam library and can be asserted in the SAMLRequestEvent
func TestCommands_AuthFromProvider_SAML(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			res{
				...
				values: map[string][]string{
					"SAMLRequest": nil, // generated IDs so not assertable
					"RelayState":  {"id"},
				},
			},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			headers, _, err := c.AuthFromProvider(tt.args.ctx, tt.args.idpID, tt.args.state, tt.args.callbackURL, tt.args.samlRootURL)
			require.ErrorIs(t, err, tt.res.err)

			authURL, err := url.Parse(headers.Get("Location"))
