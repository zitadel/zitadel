...
func (c *Commands) ChangeUserMachine(ctx context.Context, machine *ChangeMachine, check PermissionCheck) (err error) {
	...
	if machine.Changed() && check != nil {
		if err := check(existingMachine.ResourceOwner, existingMachine.AggregateID); err != nil {
			return err
		}
	}
