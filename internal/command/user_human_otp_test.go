...
func TestCommands_createHumanTOTP(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				ctx:           http_util.WithRequestedHost(authz.NewMockContext("instanceID", "org1", "user1", language.English), "zitadel.com"),
				resourceOwner: "org1",
				userID:        "user1",
			},
			want: true,
...
func TestCommands_HumanCheckMFATOTPSetup(t *testing.T) {
	ctx := authz.NewMockContext("", "org1", "user1", language.English)
...
func TestCommandSide_AddHumanOTPSMS(t *testing.T) {
	ctx := authz.NewMockContext("inst1", "org1", "user1", language.English)
	type fields struct {
