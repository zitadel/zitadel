...
func newZitadelTranslator(defaultLanguage language.Tag, allowedLanguages []language.Tag) (*i18n.Translator, error) {
	return translatorFromNamespace("zitadel", defaultLanguage, allowedLanguages)
}

func translatorFromNamespace(namespace string, defaultLanguage language.Tag, allowedLanguages []language.Tag) (*i18n.Translator, error) {
	dir, err := fs.NewWithNamespace(namespace)
	logging.WithFields("namespace", namespace).OnError(err).Panic("unable to get namespace")

	return i18n.NewTranslator(dir, defaultLanguage, allowedLanguages, "")
}
