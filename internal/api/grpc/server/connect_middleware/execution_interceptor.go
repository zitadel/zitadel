...
func ExecutionHandler(alg crypto.EncryptionAlgorithm, queries execution.SigningKey) connect.UnaryInterceptorFunc {
	return func(handler connect.UnaryFunc) connect.UnaryFunc {
		return func(ctx context.Context, req connect.AnyRequest) (_ connect.AnyResponse, err error) {
			...
			handledReq, err := executeTargetsForRequest(ctx, requestTargets, req.Spec().Procedure, req, alg, queries)
			if err != nil {
				return nil, err
			}
			...
			return executeTargetsForResponse(ctx, responseTargets, req.Spec().Procedure, handledReq, response, alg, queries)
		}
	}
}

func executeTargetsForRequest(ctx context.Context, targets []target_domain.Target, fullMethod string, req connect.AnyRequest, alg crypto.EncryptionAlgorithm, queries execution.SigningKey) (_ connect.AnyRequest, err error) {
	...
	_, err = execution.CallTargets(ctx, targets, info, alg, queries)
	if err != nil {
		return nil, err
	}
	return req, nil
...
func executeTargetsForResponse(ctx context.Context, targets []target_domain.Target, fullMethod string, req connect.AnyRequest, resp connect.AnyResponse, alg crypto.EncryptionAlgorithm, queries execution.SigningKey) (_ connect.AnyResponse, err error) {
	ctx, span := tracing.NewSpan(ctx)
	defer func() { span.EndWithError(err) }()
