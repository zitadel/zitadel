...
func (s *Server) ListTarget(ctx context.Context, req *execution.ListTargetsRequest) (*execution.ListTargetsResponse, error) {
...
func targetToPb(t *query.Target) *execution.Target {
	target := &execution.Target{
		Details: object.DomainToDetailsPb(&domain.ObjectDetails{
			Sequence:      t.Sequence,
			EventDate:     t.ChangeDate,
			ResourceOwner: t.ResourceOwner,
		}),
		TargetId: t.ID,
		Name:     t.Name,
	...
	if t.Async {
		target.ExecutionType = &execution.Target_IsAsync{IsAsync: true}
	}
	if t.InterruptOnError {
		target.ExecutionType = &execution.Target_InterruptOnError{InterruptOnError: true}
	}
