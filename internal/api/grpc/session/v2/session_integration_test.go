...
func TestServer_CreateSession_successfulIntent(t *testing.T) {
	...
	intentID, token, _, _ := Tester.CreateSuccessfulIntent(t, idpID, User.GetUserId())
	updateResp, err := Client.SetSession(CTX, &session.SetSessionRequest{
		SessionId:    createResp.GetSessionId(),
		SessionToken: createResp.GetSessionToken(),
		Checks: &session.Checks{
...
func TestServer_CreateSession_successfulIntentUnknownUserID(t *testing.T) {
	...
	intentID, token, _, _ := Tester.CreateSuccessfulIntent(t, idpID, "")
	updateResp, err := Client.SetSession(CTX, &session.SetSessionRequest{
		SessionId:    createResp.GetSessionId(),
		SessionToken: createResp.GetSessionToken(),
		Checks: &session.Checks{
	...
	require.Error(t, err)
	Tester.CreateUserIDPlink(CTX, User.GetUserId(), User.GetUserId(), idpID, User.GetUserId())
	updateResp, err = Client.SetSession(CTX, &session.SetSessionRequest{
		SessionId:    createResp.GetSessionId(),
		SessionToken: createResp.GetSessionToken(),
		Checks: &session.Checks{
