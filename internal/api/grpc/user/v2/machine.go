...
func (s *Server) updateUserTypeMachine(ctx context.Context, machinePb *user.UpdateUserRequest_Machine, userId string, userName *string, check command.PermissionCheck) (*user.UpdateUserResponse, error) {
	cmd := updateMachineUserToCommand(userId, userName, machinePb)
	err := s.command.ChangeUserMachine(ctx, cmd, check)
	if err != nil {
		return nil, err
	}
