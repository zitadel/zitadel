...
func (s *Server) updateUserTypeHuman(ctx context.Context, humanPb *user.UpdateUserRequest_Human, userId string, userName *string, check command.PermissionCheck) (*user.UpdateUserResponse, error) {
	cmd, err := updateHumanUserToCommand(userId, userName, humanPb)
	if err != nil {
		return nil, err
	}
	if err = s.command.ChangeUserHuman(ctx, cmd, s.userCodeAlg, check); err != nil {
		return nil, err
	}
