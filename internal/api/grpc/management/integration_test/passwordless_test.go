...
func TestPasswordlessRegistration_CrossOrg(t *testing.T) {
	...
	orgB := Instance.CreateOrganization(iamOwnerCTX, fmt.Sprintf("CrossOrg-%s", gofakeit.AppName()), gofakeit.Email())
	victimID := Instance.CreateHumanUserVerified(
		iamOwnerCTX, orgB.GetOrganizationId(), gofakeit.Email(), gofakeit.Phone(),
	).GetUserId()
	...
	t.Run("org owner of the same org is allowed", func(t *testing.T) {
		ownUserID := Instance.CreateHumanUserVerified(
			iamOwnerCTX, Instance.DefaultOrg.GetId(), gofakeit.Email(), gofakeit.Phone(),
		).GetUserId()
