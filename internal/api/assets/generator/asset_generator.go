...
func GenerateAssetHandler(configFilePath, handlerPrefix string, authz, router, docs io.Writer) {
	...
	logging.OnError(err).Fatal("cannot read config")
	tmplAuthz, err := template.New("").Parse(authzTmpl)
	logging.OnError(err).Fatal("cannot parse authz template")
	tmplRouter, err := template.New("").Parse(routerTmpl)
	logging.OnError(err).Fatal("cannot parse router template")
	tmplDocs, err := template.New("").Parse(docsTmpl)
	logging.OnError(err).Fatal("cannot parse docs template")
	data := &struct {
		GoPkgName string
		Name      string
		Prefix    string
	...
	logging.OnError(err).Fatal("cannot generate authz")
	err = tmplRouter.Execute(router, data)
	logging.OnError(err).Fatal("cannot generate router")
	err = tmplDocs.Execute(docs, data)
	logging.OnError(err).Fatal("cannot generate docs")
}
