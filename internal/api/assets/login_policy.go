...
func (l *labelPolicyLogoUploader) Callback(ctx context.Context, info *domain.AssetInfo, orgID string, commands *command.Commands) error {
	if l.defaultPolicy {
		if l.darkMode {
			_, err := commands.AddLogoDarkDefaultLabelPolicy(ctx, authz.GetInstance(ctx).InstanceID(), info.Key)
			return err
		}
		_, err := commands.AddLogoDefaultLabelPolicy(ctx, authz.GetInstance(ctx).InstanceID(), info.Key)
		return err
	}
	if l.darkMode {
...
func (l *labelPolicyIconUploader) Callback(ctx context.Context, info *domain.AssetInfo, orgID string, commands *command.Commands) error {
	if l.defaultPolicy {
		if l.darkMode {
			_, err := commands.AddIconDarkDefaultLabelPolicy(ctx, authz.GetInstance(ctx).InstanceID(), info.Key)
			return err
		}
		_, err := commands.AddIconDefaultLabelPolicy(ctx, authz.GetInstance(ctx).InstanceID(), info.Key)
		return err
	}
...
func (l *labelPolicyFontUploader) Callback(ctx context.Context, info *domain.AssetInfo, orgID string, commands *command.Commands) error {
	if l.defaultPolicy {
		_, err := commands.AddFontDefaultLabelPolicy(ctx, authz.GetInstance(ctx).InstanceID(), info.Key)
		return err
	}
