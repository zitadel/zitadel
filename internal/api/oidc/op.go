...
func NewProvider(ctx context.Context, config Config, defaultLogoutRedirectURI string, externalSecure bool, command *command.Commands, query *query.Queries, repo repository.Repository, encryptionAlg crypto.EncryptionAlgorithm, cryptoKey []byte, es *eventstore.Eventstore, projections *database.DB, callDurationInterceptor, userAgentCookie, instanceHandler, accessHandler func(http.Handler) http.Handler) (op.OpenIDProvider, error) {
	...
	options, err := createOptions(config, externalSecure, callDurationInterceptor, userAgentCookie, instanceHandler, accessHandler)
	if err != nil {
...
func createOptions(config Config, externalSecure bool, callDurationInterceptor, userAgentCookie, instanceHandler, accessHandler func(http.Handler) http.Handler) ([]op.Option, error) {
