...
func (o *OPStorage) CreateAccessToken(ctx context.Context, req op.TokenRequest) (_ string, _ time.Time, err error) {
	...
	case *AuthRequestV2:
		...
		activity.SetStorageInfo(ctx, "", authReq.CurrentAuthRequest.UserID, activity.OIDCAccessToken)
	...
	// trigger activity log for authentication for user
	activity.SetStorageInfo(ctx, userOrgID, req.GetSubject(), activity.OIDCAccessToken)
...
func (o *OPStorage) CreateAccessAndRefreshTokens(ctx context.Context, req op.TokenRequest, refreshToken string) (_, _ string, _ time.Time, err error) {
	...
	case *AuthRequestV2:
		// trigger activity log for authentication for user
		activity.SetStorageInfo(ctx, "", tokenReq.GetSubject(), activity.OIDCRefreshToken)
	...
	// trigger activity log for authentication for user
	activity.SetStorageInfo(ctx, userOrgID, req.GetSubject(), activity.OIDCRefreshToken)
...
func (o *OPStorage) TokenRequestByRefreshToken(ctx context.Context, refreshToken string) (_ op.RefreshTokenRequest, err error) {
	...
	if strings.HasPrefix(plainToken, command.IDPrefixV2) {
		...
		// trigger activity log for authentication for user
		activity.SetStorageInfo(ctx, "", oidcSession.UserID, activity.OIDCRefreshToken)
	...
	activity.SetStorageInfo(ctx, tokenView.ResourceOwner, tokenView.UserID, activity.OIDCRefreshToken)
