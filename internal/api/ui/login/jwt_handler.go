...
func (l *Login) handleJWTExtraction(w http.ResponseWriter, r *http.Request, authReq *domain.AuthRequest, identityProvider *query.IDPTemplate) {
	...
	if err != nil {
		if _, actionErr := l.runPostExternalAuthenticationActions(&domain.ExternalUser{}, nil, authReq, r, nil, err); actionErr != nil {
	...
	if err != nil {
		if _, actionErr := l.runPostExternalAuthenticationActions(&domain.ExternalUser{}, tokens(session), authReq, r, user, err); actionErr != nil {
			logging.WithError(err).Error("both external user authentication and action post authentication failed")
		}
		l.renderError(w, r, authReq, err)
		return
