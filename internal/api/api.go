...
func New(
	...
	api.grpcServer = server.CreateServer(api.verifier, authZ, queries, tlsConfig, accessInterceptor.AccessService())
	api.grpcGateway, err = server.CreateGateway(ctx, port, accessInterceptor)
	if err != nil {
		return nil, err
	}
