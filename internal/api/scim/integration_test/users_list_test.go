	...
	tests := []struct {
		...
		},
		{
			...
			assert: func(t assert.TestingT, resp *scim.ListResponse[*resources.ScimUser]) {
				...
				assert.True(t, strings.HasPrefix(resp.Resources[0].UserName, "scim-username-1: "), "got %q", resp.Resources[0].UserName)
				assert.True(t, strings.HasPrefix(resp.Resources[1].UserName, "scim-username-2: "), "got %q", resp.Resources[1].UserName)
			},
		},
		{
