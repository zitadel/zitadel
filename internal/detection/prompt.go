...
func buildPrompt(signal signals.Signal, snapshot signals.Snapshot, maxEvents int) (llm.Prompt, error) {
	if maxEvents <= 0 {
		maxEvents = 1
	}
	...
	if err != nil {
		return llm.Prompt{}, fmt.Errorf("marshal risk prompt context: %w", err)
	}
	return llm.Prompt{System: systemPrompt, User: string(contextJSON)}, nil
}

func promptSignalFromSignal(signal signals.Signal, findings []signals.RecordedFinding) promptSignal {
	ps := promptSignal{
...
func deterministicFindingNames(findings []signals.RecordedFinding) []string {
