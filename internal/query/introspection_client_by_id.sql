with config as (
		select instance_id, app_id, client_id, client_secret, 'api' as app_type, minimal_introspection
		from projections.apps7_api_configs
		where instance_id = $1
			and client_id = $2
	union all
		select instance_id, app_id, client_id, client_secret, 'oidc' as app_type, minimal_introspection
...
select c.app_id, c.client_id, c.client_secret, c.app_type,
       ...
       k.public_keys, c.minimal_introspection
from config c
