...
func NewGroupMetadataValueSearchQuery(value []byte, comparison BytesComparison) (SearchQuery, error) {
	return NewBytesQuery(GroupMetadataValueCol, value, comparison)
}

func NewGroupMetadataKeyAndValueSearchQuery(key string, keyComparison TextComparison, value []byte, valueComparison BytesComparison) (*AndQuery, error) {
	keyQuery, err := NewTextQuery(GroupMetadataKeyCol, key, keyComparison)
	if err != nil {
		return nil, err
	}
	valueQuery, err := NewBytesQuery(GroupMetadataValueCol, value, valueComparison)
	if err != nil {
		return nil, err
	}
	return NewAndQuery(keyQuery, valueQuery)
}

// SearchAllGroupMetadata searches metadata across all groups in the organization
func (q *Queries) SearchAllGroupMetadata(ctx context.Context, shouldTriggerBulk bool, queries *GroupMetadataSearchQueries) (metadata *GroupMetadataList, err error) {
	ctx, span := tracing.NewSpan(ctx)
	defer func() { span.EndWithError(err) }()
	...
	stmt, args, err := queries.toQuery(query).Where(eq).ToSql()
	if err != nil {
		return nil, zerrors.ThrowInternal(err, "QUERY-Hgb3d", "Errors.Query.SQLStatment")
	}
