...
func (p *instanceRelationalProjection) reduceDefaultOrgSet(event eventstore.Event) (*handler.Statement, error) {
	...
	return handler.NewStatement(e, func(ctx context.Context, ex handler.Executer, projectionName string) error {
		...
		repo := repository.InstanceRepository()
		return p.updateInstance(ctx, v3_sql.SQLTx(tx), event, repo, repo.SetDefaultOrganization(e.OrgID))
	}), nil
}
...
func (p *instanceRelationalProjection) reduceManagementConsoleSet(event eventstore.Event) (*handler.Statement, error) {
	...
	return handler.NewStatement(e, func(ctx context.Context, ex handler.Executer, projectionName string) error {
		...
		repo := repository.InstanceRepository()
		return p.updateInstance(ctx, v3_sql.SQLTx(tx), event, repo, repo.SetManagementConsoleClientID(e.ClientID), repo.SetManagementConsoleApplicationID(e.AppID))
	}), nil
}
