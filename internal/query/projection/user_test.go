		...
		},
		{
			...
			want: wantReduce{
				...
				executer: &testExecuter{
					executions: []execution{
						{
							expectedStmt: "UPDATE projections.users2 SET change_date=$1, state=$2, sequence=$3 WHERE (id = $4) AND (instance_id = $5)",
							expectedArgs: []interface{}{
								anyArg{},
		...
		},
		{
			...
			want: wantReduce{
				...
				executer: &testExecuter{
					executions: []execution{
						{
							expectedStmt: "UPDATE projections.users2 SET change_date=$1, username=$2, sequence=$3 WHERE (id = $4) AND (instance_id = $5)",
							expectedArgs: []interface{}{
								anyArg{},
								"username",
								uint64(15),
		...
		},
		{
			...
			want: wantReduce{
				...
				executer: &testExecuter{
					executions: []execution{
						{
							expectedStmt: "UPDATE projections.users2 SET change_date=$1, sequence=$2 WHERE (id = $3) AND (instance_id = $4)",
							expectedArgs: []interface{}{
								anyArg{},
								uint64(15),
								"agg-id",
						...
						{
							expectedStmt: "UPDATE projections.users2_humans SET first_name=$1, last_name=$2, nick_name=$3, display_name=$4, preferred_language=$5, gender=$6 WHERE (user_id = $7) AND (instance_id = $8)",
							expectedArgs: []interface{}{
								"first-name",
								"last-name",
								"nick-name",
			...
			want: wantReduce{
				...
				executer: &testExecuter{
					executions: []execution{
						...
						{
							expectedStmt: "UPDATE projections.users2_humans SET phone=$1, is_phone_verified=$2 WHERE (user_id = $3) AND (instance_id = $4)",
							expectedArgs: []interface{}{
								"+41 00 000 00 00",
								false,
								"agg-id",
			...
			want: wantReduce{
				...
				executer: &testExecuter{
					executions: []execution{
						...
						{
							expectedStmt: "UPDATE projections.users2_notifications SET last_phone=$1, verified_phone=$2 WHERE (user_id = $3) AND (instance_id = $4)",
							expectedArgs: []interface{}{
								nil,
								nil,
								"agg-id",
			...
			want: wantReduce{
				...
				executer: &testExecuter{
					executions: []execution{
						...
						{
							expectedStmt: "INSERT INTO projections.users2_notifications (user_id, instance_id, verified_phone) SELECT user_id, instance_id, last_phone FROM projections.users2_notifications AS copy_table WHERE copy_table.user_id = $1 AND copy_table.instance_id = $2 ON CONFLICT (user_id, instance_id) DO UPDATE SET (user_id, instance_id, verified_phone) = (EXCLUDED.user_id, EXCLUDED.instance_id, EXCLUDED.last_phone)",
							expectedArgs: []interface{}{
								"agg-id",
								"instance-id",
							},
			...
			want: wantReduce{
				...
				executer: &testExecuter{
					executions: []execution{
						...
						{
							expectedStmt: "UPDATE projections.users2_humans SET email=$1, is_email_verified=$2 WHERE (user_id = $3) AND (instance_id = $4)",
							expectedArgs: []interface{}{
								"email@zitadel.com",
								false,
								"agg-id",
			...
			want: wantReduce{
				...
				executer: &testExecuter{
					executions: []execution{
						...
						{
							expectedStmt: "INSERT INTO projections.users2_notifications (user_id, instance_id, verified_email) SELECT user_id, instance_id, last_email FROM projections.users2_notifications AS copy_table WHERE copy_table.user_id = $1 AND copy_table.instance_id = $2 ON CONFLICT (user_id, instance_id) DO UPDATE SET (user_id, instance_id, verified_email) = (EXCLUDED.user_id, EXCLUDED.instance_id, EXCLUDED.last_email)",
							expectedArgs: []interface{}{
								"agg-id",
								"instance-id",
							},
			...
			want: wantReduce{
				...
				executer: &testExecuter{
					executions: []execution{
						...
						{
							expectedStmt: "UPDATE projections.users2_machines SET name=$1, description=$2 WHERE (user_id = $3) AND (instance_id = $4)",
							expectedArgs: []interface{}{
								"machine-name",
								"description",
								"agg-id",
