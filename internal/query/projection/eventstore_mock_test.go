...
func (m *mockEventStore) appendPushResponse(events []eventstore.Event) *mockEventStore {
	m.pushResponse = append(m.pushResponse, events)
	return m
}

func (m *mockEventStore) appendInstanceIDsResponse(ids []string) *mockEventStore {
	m.instanceIDsResponse = append(m.instanceIDsResponse, ids)
	return m
}
