			...
			want: wantReduce{
				...
				executer: &testExecuter{
					executions: []execution{
						{
							expectedStmt: "INSERT INTO zitadel.projections.org_members (org_id, user_id, roles, creation_date, change_date, sequence, resource_owner) VALUES ($1, $2, $3, $4, $5, $6, $7)",
		...
		},
		{
			...
			want: wantReduce{
				...
				executer: &testExecuter{
					executions: []execution{
						{
							expectedStmt: "UPDATE zitadel.projections.org_members SET (roles, change_date, sequence) = ($1, $2, $3) WHERE (org_id = $4) AND (user_id = $5)",
							expectedArgs: []interface{}{
								[]string{"role", "changed"},
								anyArg{},
								uint64(15),
		...
		},
		{
			...
			want: wantReduce{
				...
				executer: &testExecuter{
					executions: []execution{
						{
							expectedStmt: "DELETE FROM zitadel.projections.org_members WHERE (org_id = $1) AND (user_id = $2)",
							expectedArgs: []interface{}{
								"agg-id",
								"user-id",
							},
