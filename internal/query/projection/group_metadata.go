...
func (*groupMetadataProjection) Init() *old_handler.Check {
	return handler.NewTableCheck(
		handler.NewTable([]*handler.InitColumn{
			...
			handler.WithIndex(handler.NewIndex("resource_owner", []string{GroupMetadataColumnResourceOwner})),
		),
	)
}
...
func (p *groupMetadataProjection) reduceMetadataSet(event eventstore.Event) (*handler.Statement, error) {
	e, err := assertEvent[*group.MetadataSetEvent](event)
	if err != nil {
		return nil, err
	}
...
func (p *groupMetadataProjection) reduceMetadataRemoved(event eventstore.Event) (*handler.Statement, error) {
	e, err := assertEvent[*group.MetadataRemovedEvent](event)
	if err != nil {
		return nil, err
	}
...
func (p *groupMetadataProjection) reduceMetadataRemovedAll(event eventstore.Event) (*handler.Statement, error) {
	...
	case *group.MetadataRemovedAllEvent,
		*group.GroupRemovedEvent:
		//ok
	default:
		return nil, zerrors.ThrowInvalidArgumentf(nil, "HANDL-Cmnf2", "reduce.wrong.event.type %v", []eventstore.EventType{group.MetadataRemovedAllType, group.GroupRemovedEventType})
	}
	return handler.NewDeleteStatement(
		event,
		[]handler.Condition{
