...
const (
	...
	UseDefaultRedirectUriForNotificationLinksCol = "use_default_redirect_uri_for_notification_links"
)
...
func (*loginPolicyProjection) Init() *old_handler.Check {
	return handler.NewTableCheck(
		handler.NewTable([]*handler.InitColumn{
			...
			handler.NewColumn(UseDefaultRedirectUriForNotificationLinksCol, handler.ColumnTypeBool, handler.Default(false)),
		},
...
func (p *loginPolicyProjection) reduceLoginPolicyAdded(event eventstore.Event) (*handler.Statement, error) {
	...
	return handler.NewCreateStatement(&policyEvent, []handler.Column{
		...
		handler.NewCol(UseDefaultRedirectUriForNotificationLinksCol, policyEvent.UseDefaultRedirectUriForNotificationLinks),
	}), nil
}
...
func (p *loginPolicyProjection) reduceLoginPolicyChanged(event eventstore.Event) (*handler.Statement, error) {
	...
	if policyEvent.UseDefaultRedirectUriForNotificationLinks != nil {
		cols = append(cols, handler.NewCol(UseDefaultRedirectUriForNotificationLinksCol, *policyEvent.UseDefaultRedirectUriForNotificationLinks))
	}
