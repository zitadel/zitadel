...
func newPersonalAccessTokenProjection(ctx context.Context, config crdb.StatementHandlerConfig) *personalAccessTokenProjection {
	...
	config.InitCheck = crdb.NewTableCheck(
		crdb.NewTable([]*crdb.Column{
			...
			crdb.WithIndex(crdb.NewIndex("pat_user_idx3", []string{PersonalAccessTokenColumnUserID})),
			crdb.WithIndex(crdb.NewIndex("pat_ro_idx3", []string{PersonalAccessTokenColumnResourceOwner})),
		),
	)
