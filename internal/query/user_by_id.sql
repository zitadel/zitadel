WITH login_names AS (SELECT 
  ...
  JOIN lateral (
    ...
    WHERE
      ...
      AND (
        (p.is_default IS TRUE AND p.instance_id = '262030315386627324')
        OR (p.instance_id = '262030315386627324' AND p.resource_owner = u.resource_owner)
      )
    ORDER BY is_default
    LIMIT 1
  ) p ON TRUE
  ...
  WHERE
    u.instance_id = '262030315386627324'
    AND u.id = '262030315386758396'
)
SELECT 
  u.id
  , u.creation_date
...
WHERE 
  u.id = '262030315386758396'
  AND u.instance_id = '262030315386627324'
LIMIT 1
;