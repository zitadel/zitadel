...
func TestQueries_GetProjectByOIDClientID(t *testing.T) {
	expQuery := regexp.QuoteMeta(oidcClientProjectQuery)
	...
	tests := []struct {
		...
		{
			name: "found",
			mock: mockQuery(expQuery, cols, []driver.Value{"projectID"}, "instanceID", "clientID"),
			want: "projectID",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			execMock(t, tt.mock, func(db *sql.DB) {
				...
				ctx := authz.NewMockContext("instanceID", "orgID", "loginClient")
				got, err := q.GetProjectIDByOIDClientID(ctx, "clientID")
				require.ErrorIs(t, err, tt.wantErr)
				assert.Equal(t, tt.want, got)
			})
		})
