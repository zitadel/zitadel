...
func UserLockByID(db *gorm.DB, table, userID string) (*model.UserLockView, error) {
	...
	query := repository.PrepareGetByKey(table, model.UserLockSearchKey(usr_model.UserLockSearchKeyUserID), userID)
	err := query(db, user)
...
func DeleteUserLock(db *gorm.DB, table, userID string) error {
	delete := repository.PrepareDeleteByKey(table, model.UserLockSearchKey(usr_model.UserLockSearchKeyUserID), userID)
	return delete(db)
}
