...
type CacheConfig struct {
	Type   string
	Config *cache.Config `mapstructure:"config"`
}
...
func (c *CacheConfig) UnmarshalMap(value interface{}) error {
	var (
	...
	c.Type = cfg["type"].(string)

	var err error
	c.Config, err = newCacheConfig(c.Type, cfg["config"].([]byte))
	if err != nil {
...
func (c *CacheConfig) UnmarshalText(data []byte) error {
	var rc struct {
		Type   string
		Config json.RawMessage
	}
