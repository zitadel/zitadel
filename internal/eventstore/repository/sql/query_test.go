...
func Test_prepareColumns(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			res: res{
				query: "SELECT creation_date, event_type, event_sequence, previous_aggregate_sequence, previous_aggregate_root_sequence, event_data, editor_service, editor_user, resource_owner, aggregate_type, aggregate_id, aggregate_version FROM eventstore.events",
...
func Test_query_events_mocked(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{
				mock: newMockClient(t).expectQuery(t,
					`SELECT creation_date, event_type, event_sequence, previous_aggregate_sequence, previous_aggregate_root_sequence, event_data, editor_service, editor_user, resource_owner, aggregate_type, aggregate_id, aggregate_version FROM eventstore.events WHERE \( aggregate_type = \$1 \) ORDER BY event_sequence DESC`,
		...
		},
		{
			...
			fields: fields{
				mock: newMockClient(t).expectQuery(t,
					`SELECT creation_date, event_type, event_sequence, previous_aggregate_sequence, previous_aggregate_root_sequence, event_data, editor_service, editor_user, resource_owner, aggregate_type, aggregate_id, aggregate_version FROM eventstore.events WHERE \( aggregate_type = \$1 \) ORDER BY event_sequence LIMIT \$2`,
		...
		},
		{
			...
			fields: fields{
				mock: newMockClient(t).expectQuery(t,
					`SELECT creation_date, event_type, event_sequence, previous_aggregate_sequence, previous_aggregate_root_sequence, event_data, editor_service, editor_user, resource_owner, aggregate_type, aggregate_id, aggregate_version FROM eventstore.events WHERE \( aggregate_type = \$1 \) ORDER BY event_sequence DESC LIMIT \$2`,
					[]driver.Value{repository.AggregateType("user"), uint64(5)},
				),
			},
			res: res{
		...
		},
		{
			...
			fields: fields{
				mock: newMockClient(t).expectQuery(t,
					`SELECT creation_date, event_type, event_sequence, previous_aggregate_sequence, previous_aggregate_root_sequence, event_data, editor_service, editor_user, resource_owner, aggregate_type, aggregate_id, aggregate_version FROM eventstore.events WHERE \( aggregate_type = \$1 \) OR \( aggregate_type = \$2 AND aggregate_id = \$3 \) ORDER BY event_sequence DESC LIMIT \$4`,
