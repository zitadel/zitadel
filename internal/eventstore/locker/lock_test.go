...
func (db *dbMock) expectRenew(lockerID, view string, affectedRows int64) *dbMock {
	query := db.mock.
		ExpectExec(`INSERT INTO table\.locks \(locker_id, locked_until, object_type\) VALUES \(\$1, now\(\)\+\$2, \$3\) ON CONFLICT \(object_type\) DO UPDATE SET locker_id = \$4, locked_until = now\(\)\+\$5 WHERE locks\.object_type = \$6 AND \(locks\.locker_id = \$7 AND locks\.locked_until >= now\(\) OR locks\.locked_until < now\(\)\)`).
