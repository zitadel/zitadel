...
// LatestCreationDate filters the creation date for the given search query
func (es *Eventstore) LatestCreationDate(ctx context.Context, queryFactory *SearchQueryBuilder) (time.Time, error) {
	query, err := queryFactory.build(authz.GetInstance(ctx).InstanceID())
	if err != nil {
		return time.Time{}, err
	}
	return es.repo.LatestCreationDate(ctx, query)
}
