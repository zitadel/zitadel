...
//	}()
//
//	return h.bulk(ctx, lock, unlock, instanceIDs)
//}
//
...
//	defer cancel()
//
//	errs := lock(ctx, h.requeueAfter, instanceIDs...)
...
//
//	execErr := h.execBulk(ctx, instanceIDs)
...
type executeBulk func(ctx context.Context, instanceIDs []string) error
...
//	update Update,
//) executeBulk {
//	return func(ctx context.Context, instanceIDs []string) error {
//		for {
//			select {
//			case <-ctx.Done():
//				return nil
//			default:
//				hasLimitExeeded, err := h.fetchBulkStmts(ctx, query, reduce, instanceIDs)
...
//	eventQuery, eventsLimit, err := query(ctx, instanceIDs)
//	if err != nil {
