...
// 	currentStates, err := h.currentSequences(ctx, h.client.QueryContext, instanceIDs)
// 	if err != nil {
// 		return nil, 0, err
// 	}
...
// 				continue
// 			}
// 			lastEvent = state.eventCreationDate
// 			break
// 		}
// 		queryBuilder.
// 			AddQuery().
// 			AggregateTypes(h.aggregates...).
// 			CreationDateAfter(lastEvent.Add(-1 * time.Microsecond)).
...
// 			if instance == stmt.InstanceID {
// 				instanceFound = true
// 				break
// 			}
// 		}
// 		instanceIDs = append(instanceIDs, stmt.InstanceID)

// 		return append(instances, id)
// 		instanceIDs = appendToInstanceIDs(instanceIDs, stmt.InstanceID)
// 	}

// // TODO: check if there are events between  and stmts[0]
...
// func (h *StatementHandler) fetchPreviousStmts(ctx context.Context, tx *sql.Tx, stmtSeq uint64, instanceID string, sequences currentState, reduce handler.Reduce) (previousStmts []*handler.Statement, err error) {
...
// func updateSequences(sequences currentState, stmt *handler.Statement) {
// 	for _, sequence := range sequences[stmt.AggregateType] {
...
// 		}
// 	}
// 	sequences[stmt.AggregateType] = append(sequences[stmt.AggregateType], &instanceState{
// 		instanceID:        stmt.InstanceID,
// 		eventCreationDate: stmt.Sequence,
// 	})
// }
