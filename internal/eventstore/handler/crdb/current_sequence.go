...
const (
	latestEventStmtFormat     = `SELECT event_creation_date, aggregate_type, instance_id FROM %s WHERE projection_name = $1 AND instance_id = ANY ($2) FOR UPDATE`
	updateEventIDStmtFormat   = `INSERT INTO %s (projection_name, aggregate_type, instance_id, event_creation_date) VALUES `
	updateEventIDConflictStmt = ` ON CONFLICT (projection_name, aggregate_type, instance_id) DO UPDATE SET event_creation_date = EXCLUDED.event_creation_date`
)
...
func (h *StatementHandler) currentSequences(ctx context.Context, query func(context.Context, string, ...interface{}) (*sql.Rows, error), instanceIDs database.StringArray) (events, error) {
	...
	for rows.Next() {
		...
		err = rows.Scan(&creationDate, &aggregateType, &instanceID)
		if err != nil {
