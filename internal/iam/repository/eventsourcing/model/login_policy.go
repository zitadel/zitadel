...
type IdpProvider struct {
	models.ObjectRoot
	IdpProviderID   string     `json:"idpProviderId"`
	Type            int32      `json:"idpProviderType"`
	IdpConfigID     string     `json:"idpConfigId"`
	IdpCustomConfig *IdpConfig `json:"idpConfig"`
}
...
func IdpProviderToModel(provider *IdpProvider) *iam_model.IdpProvider {
	return &iam_model.IdpProvider{
		...
		IdpProviderID:   provider.IdpProviderID,
		Type:            iam_model.IdpProviderType(provider.Type),
		IdpConfigID:     provider.IdpConfigID,
		IdpCustomConfig: IdpConfigToModel(provider.IdpCustomConfig),
	}
}
...
func IdpProviderFromModel(provider *iam_model.IdpProvider) *IdpProvider {
	return &IdpProvider{
		...
		IdpConfigID:     provider.IdpConfigID,
		IdpCustomConfig: IdpConfigFromModel(provider.IdpCustomConfig),
	}
}
...
func (i *Iam) appendRemoveLoginPolicyEvent() {
	i.DefaultLoginPolicy = nil
}
