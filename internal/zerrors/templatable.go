...
type TemplatableError struct {
	Parent error
	Vars   map[string]interface{}
}

// WithTemplateVariables accepts variables which can be used to add data to error message templates using errors.As() and GetVars()
...
func WithTemplateVariables(err error, vars map[string]interface{}) error {
	if err == nil {
		return nil
	}
	return &TemplatableError{Vars: vars, Parent: err}
}

func (err *TemplatableError) Error() string {
	if err.Parent != nil {
...
func (err *TemplatableError) Unwrap() error {
	return err.Parent
}

// GetVars returns a flat map of all vars added by WithVars
func (err *TemplatableError) GetVars() map[string]interface{} {
	var vars map[string]interface{}
	p := new(TemplatableError)
	if err.Parent != nil {
