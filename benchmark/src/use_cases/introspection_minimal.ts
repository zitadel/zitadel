...
import { createAPI, createAppKey, setMinimalIntrospection } from '../app';
...
// Same scenario as introspection.ts, except every API app has minimal_introspection enabled.
// Run both benchmarks with identical VUS/DURATION and compare the oidc_introspect_duration
// trend to measure the effect of skipping the userinfo/claims lookup on introspection.
export async function setup() {
  ...
  await Promise.all(
    apis.map((api, i) => {
      return setMinimalIntrospection(api.appId, projects[i].id, org, adminTokens.accessToken!);
    }),
  );
  console.info(`setup: minimal introspection enabled on ${apis.length} apis`);
