...
func StartRedis(ctx context.Context, logw io.Writer) (*redis.RedisContainer, *RedisConfig, error) {
	// The redis module already waits for "Ready to accept connections" by default.
	container, err := redis.Run(ctx, "redis:8",
		testcontainers.WithLogger(log.New(logw, "[testcontainers/redis] ", log.LstdFlags)),
	)
	if err != nil {
