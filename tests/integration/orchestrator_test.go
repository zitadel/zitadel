...
func TestIntegration(t *testing.T) {
	...
	// Reduce log noise in test output; only warnings and errors are shown.
	t.Setenv("ZITADEL_LOG_LEVEL", "warn")
	...
	packageFilter := os.Getenv("INTEGRATION_PACKAGE_FILTER")
	packages, skippedPkgs, filteredPkgs, err := discoverIntegrationPackages(packageFilter)
	require.NoError(t, err, "discover packages")
	logf("Found %d integration test packages (%d v2beta skipped, %d filtered out)", len(packages), len(skippedPkgs), len(filteredPkgs))
	if packageFilter != "" {
		logf("Package filter: %s", packageFilter)
	}
	for _, s := range skippedPkgs {
		logf("  skipping v2beta: %s", s)
	}
	for _, s := range filteredPkgs {
		logf("  filtered out: %s", s)
	}
...
// It returns the packages to run, the packages that were skipped (v2beta), and the
// packages that were filtered out by the filter parameter.
//
// The filter parameter is a comma-separated list of substrings. When non-empty, a package
// must contain at least one of the substrings to be included. This allows grouping packages
// for CI matrix splitting, e.g. INTEGRATION_PACKAGE_FILTER="action,session" will only
// include packages whose import path contains "action" or "session".
func discoverIntegrationPackages(filter string) (packages []string, skipped []string, filtered []string, err error) {
	...
	for _, line := range strings.Split(strings.TrimSpace(string(out)), "\n") {
		if strings.Contains(line, "integration_test") || strings.Contains(line, "events_testing") {
			...
			// Apply filter if set.
			if len(filters) > 0 && !matchesAny(line, filters) {
	...
	if len(packages) == 0 {
		return nil, nil, nil, fmt.Errorf("no integration test packages found (filter=%q)", filter)
	}
	return packages, skipped, filtered, nil
}
