...
func Save(cfg *Config) error {
	...
	// Lock down permissions since the config contains raw tokens.
...
func ActiveCtx(cfg *Config) (*Context, string, error) {
	...
	if name == "" && len(cfg.Contexts) == 0 {
		...
		return nil, "", fmt.Errorf("no active context configured; set one with 'zitadel-cli context use <name>' or set both ZITADEL_INSTANCE and ZITADEL_TOKEN")
	}
	...
	if err := ctx.Validate(); err != nil {
		return nil, name, fmt.Errorf("invalid context %q: %w", name, err)
	}
