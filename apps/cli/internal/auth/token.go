...
func TokenSource(_ context.Context, cfg *config.Context) (oauth2.TokenSource, error) {
	...
	return nil, fmt.Errorf("no token available; run 'zitadel-cli login' or set ZITADEL_TOKEN")
}
...
func (s *interactiveTokenSource) Token() (*oauth2.Token, error) {
	...
	if s.cfg.RefreshToken == "" {
		return nil, fmt.Errorf("access token expired and no refresh token available; run 'zitadel-cli login' again")
	}

	refreshed, err := s.refresh()
	if err != nil {
		return nil, fmt.Errorf("token refresh failed (run 'zitadel-cli login' again): %w", err)
	}
