            ...
            {methods.map((method) => (
              <RadioGroup.Option
                ...
                className={({ active, checked }) =>
                  `${active ? "ring-2 ring-primary-light-500 ring-opacity-60 dark:ring-white/20" : ""} ${
                    checked
                ...
                {({ checked }: { checked: boolean }) => (
                  <>
                    <div className="flex w-full flex-col items-center text-sm">
                      ...
                      {/* @ts-expect-error: className is not typed correctly in this version */}
                      <RadioGroup.Label as="p" className={`font-medium ${checked ? "" : ""}`}>
