...
import { resendCode, verifyEmailCodeFromMockServer, verifyTOTPCode } from "./code.js";
...
test("username, password and email otp login, enter code manually", async ({ userCreator, userService, page }) => {
  ...
  await verifyEmailCodeFromMockServer(page, userCreator.username);
...
test("username, password and email otp login, resend code", async ({ userCreator, page }) => {
  ...
  const oldCode = await eventualEmailOTP(userCreator.username);
  await verifyEmailCodeFromMockServer(page, userCreator.username);
  await loginScreenExpect(page, userCreator.fullName);
});
...
test("username, password and email otp login, wrong code", async ({ userCreator, page }) => {
  ...
  await userCreator.create()
  await userCreator.addTOTPFactor();
  const c = "wrongcode";
  await loginWithPassword(page, userCreator.username, userCreator.password);
