...
<Callout type="warn" title="Prevent Settings Misconfiguration Lockouts">
  ...
  1.  **Generate a backup PAT:** Create a [Service User Personal Access Token](/guides/integrate/service-accounts/personal-access-token) with the [`IAM_OWNER`](/guides/manage/console/administrators) role to revert any login UI settings misconfigurations using the API.
