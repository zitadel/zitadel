...
<Callout type="warn" title="Prevent Misconfiguration Lockouts">
  ...
  1.  **Generate a backup PAT:** Create a [Service User Personal Access Token](/guides/integrate/service-accounts/personal-access-token) with the [`IAM_OWNER`](/guides/manage/console/administrators) role to revert any login UI misconfigurations using the API.
  2.  **Add a second Instance Administrator:** Always designate at least one [**second instance manager**](/guides/manage/console/administrators).
