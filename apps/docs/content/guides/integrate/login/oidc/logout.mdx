...
Single Logout (SLO) is the counterpart to SSO. With SLO a User (Human) can logout and terminate sessions across many applications, without actively logging out from them.
...
When a User (Human) logs out of the provider, the User (Human)'s session is terminated and the client can in turn reflect that in its behavior.
