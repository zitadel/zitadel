...
### Understanding the "Use new login UI" checkbox

The **"Use new login UI"** checkbox and **"Custom base URL for the new Login UI"** field work together to control which login version your application uses:

#### When the checkbox matters

The checkbox is only relevant when the instance-level Login V2 feature flag is **disabled**. In this scenario:

- **Checkbox unchecked**: Your application uses Login V1 (`/ui/login`)
- **Checkbox checked**: Your application uses Login V2 (`/ui/v2/login`)

This allows you to opt specific applications into Login V2 while keeping other applications on V1.

#### When the checkbox has