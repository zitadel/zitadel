...
<Callout type="warning" title="Performance with Large User Databases">
Login V2 uses a different query pattern than Login V1 for user lookup. In deployments with millions of users (5M+), this can result in significantly longer authentication times due to how the `login_names` table is queried.

If you experience performance issues with Login V2 in large-scale deployments, consider:
- Continuing to use Login V1, which remains fully supported
- Enabling [caching](/self-hosting/manage/cache) to improve query performance
- Monitoring database query performance and indexing strategies

The ZITADEL team is actively working on optimizing Login V2 performance for large user databases.
</Callout>
...
>
    ...
    <Tab value="Vercel Custom Login Deployment">
        ...
        6. Use the new login in your application.
            ...
            ![](../../../../public/img/guides/integrate/login/login-v2-app-config.png)
            2. Enable the [loginV2 feature](/reference/api/feature/zitadel.feature.v2.Feature