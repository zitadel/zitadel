...
<Callout type="caution" title="Login V2 performance at scale (5M+ users)">
Login V2 has a known performance issue on instances with large user databases. The `login_names` view used internally generates a subquery that can bypass indexes, causing login requests to take over a minute on databases with 5 million or more users.

If you are running at this scale, **use Login V1** until the issue is resolved. Login V1 remains fully supported. Track progress on [GitHub](https://github.com/zitadel/zitadel/pull/10475).
</Callout>
...
>
    <Tab value="Activate Zitadel Cloud V2 Login">
        ...
        <Callout type="info" title="How the checkbox and Custom base URL work">
        - **Use new Login UI checkbox:** This is a per-app override. It only has a visible effect when the instance-level Login V2 feature flag is **off** — it opts that specific app into V2 while other apps remain on V1. If the instance flag is already on (the default in v4+), the checkbox has no effect because all apps already use V2.
        - **Custom base URL field:** Only set this if you are self-hosting the login UI at a custom domain (e.g. `https://login.acme.com`). Leave it empty to use the built-in `/ui/v2/login`.
        </Callout>
