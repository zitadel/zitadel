...
3.  A Reverse proxy with TLS termination.
4.  Access to the [SWITCHaai Resource Registry](https://rr.aai.switch.ch/) (or your target federation's registry) to register the SATOSA Service Provider.
...
services:
  satosa:
    image: satosa/satosa:8.5.1-alpine3.22
    container_name: satosa
    # We assume a reverse proxy with TLS termination is sitting in front of SATOSA
    ports:
      - "8000:8000"
    environment:
      # General
      ...
      # ZITADEL Config
      # This URL comes from your ZITADEL SAML IdP configuration (see next section).
      ZITADEL_METADATA_URL: "https://zitadel.example.com/idps/<id>/saml/metadata"
