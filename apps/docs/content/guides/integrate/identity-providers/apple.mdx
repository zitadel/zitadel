...
<Callout>
For Apple with V1 hosted login, the callback URL ends with `/form` (e.g., `.../ui/login/login/externalidp/callback/form`) because Apple uses a Form Post to return the authorization response.

For V2, use the standard callback URL `.../idps/callback`, which already accepts POST requests.
</Callout>
