...
[Terminate Session Documentation](/reference/api/session/zitadel.session.v2.SessionService.DeleteSession)

Sessions can be terminated by either:
- the authenticated user
- an administrator, who is granted `session.delete` (e.g. ORG_OWNER) on the authenticated users organization
