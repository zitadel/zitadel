...
6. Your application starts a polling mechanism to check if the User (Human) has approved the device authorization request on the token endpoint
...
13. Your applications request to the token endpoint now receives the tokens or an error if the User (Human) denied the request.
...
If the User (Human) denies the device authorization request, you can deny the request by sending the following request:
