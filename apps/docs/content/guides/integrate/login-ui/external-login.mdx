...
For Apple Sign-In with V1 hosted login, the callback URL ends with `/form`: `https://${CUSTOM_DOMAIN}/ui/login/login/externalidp/callback/form`.
For V2, the standard callback URL `https://${CUSTOM_DOMAIN}/idps/callback` is used, as it already accepts POST requests.
</Callout>
