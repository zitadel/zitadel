...
If your organization uses enterprise email link scanners that pre-fetch URLs in emails, these scanners can inadvertently consume one-time verification codes before users click them.
To prevent this, set `NEXT_PUBLIC_DISABLE_AUTO_VERIFY_EMAIL` to `true`.
This disables automatic code redemption on page load and instead displays the verification code in a form with a Submit button, requiring explicit user action.
