...
sidebar_label: Fork and Deploy Login App
---
...
```bash
git checkout -b custom-login/vX.Y.Z vX.Y.Z
```
...
At the time of writing, current `@zitadel/client` and `@zitadel/proto` releases are not available as version-matched npm dependencies for the Login UI. The application consumes their workspace sources from the monorepo. Until version-matched packages are published, retain those sources rather than replacing the workspace dependencies with old npm versions. Progress is tracked in [zitadel/zitadel#12250](https://github.com/zitadel/zitadel/issues/12250).

## Configure a development environment

The current [contribution guide](https://github.com/zitadel/zitadel/blob/main/CONTRIBUTING.md#contribute-to-login) requires Node.js 22.x, Corepack/pnpm, and Nx. From the repository root:
...
```bash
git fetch upstream --tags
git checkout -b update-login-vX.Y.Z
git merge vX.Y.Z
```
