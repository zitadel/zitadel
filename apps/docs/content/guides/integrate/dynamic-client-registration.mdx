...
themselves as OIDC applications at runtime, instead of being created upfront in the Console or through the management API.
...
| Member | Notes |
| --- | --- |
| `redirect_uris` | Required. Validated against the same rules as applications created in the Console. |
| `token_endpoint_auth_method` | `client_secret_basic` (default), `client_secret_post` or `none`. |
