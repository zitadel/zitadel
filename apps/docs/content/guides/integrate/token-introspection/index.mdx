...
If there are client APIs or systems that need to access other protected APIs, these APIs or systems must be declared as [service users](/guides/manage/console/users-overview).
A service user is not considered an application type in Zitadel.
Read the introduction on how to [authenticate service users](/guides/integrate/service-accounts/authenticate-service-accounts).
