...
- [Add application roles via the Project Service API](/docs/reference/api/project/zitadel.project.v2.ProjectService.AddProjectRole)
- [Add administrator roles to users via the Internal Permission Service API](/docs/reference/api/internal_permission/zitadel.internal_permission.v2.InternalPermissionService.CreateAdministrator)
...
Let’s start with a user who has multiple roles in different organizations in a multi-tenanted setup. You can use the logged-in user’s token or the service account’s token to retrieve the roles assigned to this user using the [APIs listed under user authorizations/grants in the auth API](/docs/reference/api/authorization/zitadel.authorization.v2.AuthorizationService.ListAuthorizations).
