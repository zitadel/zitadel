...
Make sure that your admins have a [Administrator role](/guides/manage/console/managers) with permissions on an instance level such as "INSTANCE_OWNER"
...
#### Login with your User (Human)

Last step is that your User (Human) can login to ZITADEL and use the SSO account.
...
Your User (Human) is now ready to login with SSO.
...
When a User (Human) registers through this form, an organization and a user are created.
The User (Human) will automatically get the role "ORG_OWNER" withing ZITADEL and is able to manage the whole organization.
...
You can directly send a User (Human) with all the needed information like the profile, email, password. etc.
