...
You need a PAT from a service account in both the source and target systems. The source system is used only for the read-only export call, so it needs the read-only "Instance Owner Viewer" role, while the target system writes data on import and needs the "Instance Owner" role.
...
5. Add the export_user as [administrator](/guides/manage/console/administrators) with the role "Instance Owner Viewer"
...
3. Create a [personal access token](/guides/integrate/service-accounts/personal-access-token)
4. Go to the Default settings
5. Add the import_user as [administrator](/guides/manage/console/administrators) with the role "Instance Owner"

Save the PAT to the environment variable `PAT_IMPORT_TOKEN` and the target domain as `ZITADEL_IMPORT_DOMAIN` to run the following scripts.
