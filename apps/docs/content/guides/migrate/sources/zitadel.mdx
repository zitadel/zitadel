...
You need a PAT from a service account in both the source and target system. The source system is used only for the read-only export call, so it needs the read-only "Instance Owner Viewer" role, while the target system writes data on import and needs the "Instance Owner" role.
...
5. Add the export_user as [manager](/guides/manage/console/administrators) with the role "Instance Owner Viewer"
