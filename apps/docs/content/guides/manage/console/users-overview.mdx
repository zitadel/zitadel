...
sidebar_position: 3
---
 main:docs/docs/guides/manage/console/users-overview.mdx
import CreateUser from './_create-user.mdx';

## Overview

ZITADEL supports authentication and authorization for different user types. We mainly differentiate between **Human Users** (interactive) and **Service Users** (machine-to-machine).
...
### Human Users
Human users represent actual people who log in via an interactive interface (e.g., a login page).
...
Read more on how to [login users with ZITADEL](/guides/integrate/login/login-users). main:docs/docs/guides/manage/console/users-overview.mdx

### Service Users (Machine Accounts)
Service users represent machines, backend services, or IoT devices requiring non-interactive access.
...
To see how service users are utilized in practice, check out our [NextJS B2B Demo application](https://github.com/zitadel/zitadel-nextjs-b2b).
...
### Managers
Any user—whether Human or Service User—can be assigned an [Administrator Role](/concepts/structure/managers).
A user with an administrator role is not just an end-user of your application but also has permissions to manage specific aspects of the ZITADEL instance, organization, or project (e.g., `ORG_OWNER`, `PROJECT_OWNER`). main:docs/docs/guides/manage/console/users-overview.mdx
...
<CreateUser/> main:docs/docs/guides/manage/console/users-overview.mdx
...
- [User onboarding and registration](/guides/integrate/onboarding) main:docs/docs/guides/manage/console/users-overview.mdx
