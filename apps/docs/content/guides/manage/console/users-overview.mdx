...
ZITADEL supports authentication and authorization for different user types. We mainly differentiate between **Users (Human)** (interactive) and **Service Users** (machine-to-machine).
...
### Users (Human)
Users (Human) represent actual people who log in via an interactive interface (e.g., a login page).
...
### Managers
Any user—whether User (Human) or Service User—can be assigned an [Administrator Role](/concepts/structure/managers).
