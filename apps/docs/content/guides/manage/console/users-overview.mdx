...
ZITADEL supports authentication and authorization for different user types. We mainly differentiate between **Users (Human)** (interactive) and **Service Accounts** (machine-to-machine).
...
### Managers
Any user—whether User (Human) or Service Account—can be assigned an [Administrator Role](/concepts/structure/managers).
