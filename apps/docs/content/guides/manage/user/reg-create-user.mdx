...
If you are collecting all the User (Human) information and a password, you can directly create the User (Human) with the password.
With the `password_change_required` flag you can choose if the User (Human) has to change the password on the first login or not.
This might make sense if an administrator created the user.

## With passkey

You can directly ask for a link to create the passkey registration for the User (Human).
Fill the User (Human) data and set the attribute `request_passwordless_registration` to true.
...
- **unspecified** The User (Human) is free to choose
...
If you already have a User (Human) in ZITADEL, it is possible to add passkey later.

[Add Passkey Registration ](/apis/resources/mgmt)

Send the User (Human) id in the request and you will get a link and an expiration as response.
You can then customize the link the same as described above in the creation process.

The second possibility is to send the link directly to the User (Human) per email.
...
In case you want to create the User (Human) with an verified email address set the `isEmailVerified` property to `true`.
