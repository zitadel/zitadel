...
We recommend that you use [Authorization Code](/docs/apis/openidoauth/grant-types#authorization-code) for the OAuth 2.0 Proxy.
...
If you want oauth2-proxy to check for roles in the tokens you have to add an [action](/docs/guides/manage/console/actions-overview) in ZITADEL to [complement the token](/docs/apis/actions/complement-token) according to [this example](https://github.com/zitadel/actions/blob/main/examples/custom_roles.js) and add the following configuration to the config:
