...
We can mainly differentiate between Users (Human) and Machine Users.

### Users (Human)

Users (Human) typically logon with an interactive login.
This means that an application redirects a User (Human) to a website ("login page") where the User (Human) can provide the credentials.
...
The main difference between User (Human) and machine users is the type of credentials that can be used for authentication: Users (Human) typically logon via an login prompt, but Machine users require a non-interactive logon process.

Learn how to [use service users](/guides/integrate/service-users/authenticate-service-users) with ZITADEL.

### Managers

Any user, User (Human) or service user, can be given a [Manager](/concepts/structure/managers) role.
