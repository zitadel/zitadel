...
We can mainly differentiate between Users and Service Accounts.
Users are accounts operated/used by a human, while service accounts are those operated/used by an application/third-party software.

### Users

Users typically logon with an interactive login.
...
Service accounts are for machine-to-machine communication, and you would use those typically to access secure backend services.
For example, in ZITADEL you would require an authenticated Service Account to access the Management API.
The main difference between users and service accounts is the type of credentials that can be used for authentication: users typically logon via a login prompt, but service accounts require a non-interactive logon process.

Learn how to [use service accounts](/guides/integrate/service-accounts/authenticate-service-accounts) with ZITADEL.

### Administrators

Any user or service account can be assigned an [Administrator](/concepts/structure/administrators) role.
...
Users can log in via an external identity provider, using [identity brokering](../features/identity-brokering) ("Single-sign-on").
...
By using [external user role assignments](../features/external-user-grant) an organization is able to invite users from another organization.
...
This is why we recommend structuring users along the smallest unit of groups.
...
- [ZITADEL APIs: Users](/reference/api/user)
