...
If you are unsure of how to structure your organizations and projects, we recommend reading the guide on [solution architecture](/concepts/architecture/solution).
...
A role consists of different attributes. Only the key is relevant to the role assignment and must therefore be unique.
The display name is only to provide a human-readable name if needed.
And the group should enable a better handling in ZITADEL management console, like giving a user all the roles of a specific group. (Not implemented yet)
...
The default name of the project is "ZITADEL"; this might differ on self-hosted instances where you change the default name.
