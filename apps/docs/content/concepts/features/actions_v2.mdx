...
- A business requires domain-specific data validation before a User (Human) can be created or authenticated.
- A business needs to automate tasks, such as assigning roles to Users (Human) based on their ADFS 2016+ groups.
- A business needs to store metadata on a User (Human), which is used for integrating applications.
- A business needs to restrict which Users (Human) are allowed to register to a certain organization by their email domains.
...
- **Event:** Reacts to different events created in ZITADEL, for example, to inform someone if a User (Human) gets locked.
