...
- `Users` are the end-users of your application. Like with any CIAM solution, users should be able to perform tasks like register/join, update their profile, manage authenticators, etc. There are certain actions that can be executed pre-login, yet others require the User (Human) to have a valid session.
...
- User (Human) receives an email with a one-time code for verification purpose
- User (Human) has to enter the one-time code to finish registration
- User (Human) can re-request a new one-time code

The User (Human) is prompted on the first login to set up MFA.
...
- User (Human) can update the profile information
- An account is created within ZITADEL and linked with the external identity provider

#### Account Linking

When you log in with an external identity provider, and the User (Human) does not exist in ZITADEL, then an autoregister flow is triggered. The User (Human) is presented with two options:

- Create a new account: A new account will be created as stated above
- Autolink: The User (Human) is prompted to log in with an existing [local account](#local-account). If successful, the existing identity from the external identity provider will be linked with the local account. A User (Human) can now log in with either the local account or any of the linked external accounts.
...
- the User (Human) will be shown a button for each identity provider as an alternative to log in with a [local account](#local-account)
- when clicking the button, the User (Human) will be redirected to the identity provider
- after successful login, the User (Human) will be redirected to the application
...
ZITADEL provides a self-service UI for the User (Human) profile out-of-the box under the path _$CUSTOM_DOMAIN/ui/console/users/me_.
...
The User (Human) receives a one-time password and can verify control over the given email address.

### Phone Verification

Users can change their phone number.
The User (Human) receives a one-time password and can verify control over the given phone number.
...
The User (Human) can log in with any of the linked accounts.
[Linking of external accounts](#account-linking) is done during the login process.

## Managers

It is important to note that a `Manager` is not simply an administrative User (Human), but can be used to create much more advanced scenarios such as delegating administration of a whole organization to a User (Human), acting then as administrator and permission manager of that user group.
...
- `Managers in delegation`: Granting administrative permissions to a User (Human) from a different organization where the organizations depend on each other

A list of [Administrator Roles](../../guides/manage/console/managers#roles) is available with a description of permissions.
Managers can be assigned to both Users (Human) and service users, e.g., for managing certain tasks programmatically.

### Managers in isolation

A User (Human) with Administrator roles `IAM_OWNER` or `ORG_OWNER` might want to assign other Users from their organization elevated permissions to handle certain aspects of the IAM tasks.
...
Given such as set up the owner might want to give one administrative User (Human) of the customer organization the role `ORG_OWNER`.
Equipped with this Administrator Role, the User (Human) can perform actions like configuring their own SSO/Identity Provider, set security policy for their organization, customize branding, or assign project or Administrator roles to other Users.
