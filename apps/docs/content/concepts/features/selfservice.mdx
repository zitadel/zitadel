...
When you log in with an external identity provider, and the user does not exist in ZITADEL, then an autoregister flow is triggered. The User (Human) is presented with two options:
