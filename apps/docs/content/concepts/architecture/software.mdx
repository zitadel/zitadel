...
- [Authentication API](/apis/introduction#authentication) - allow a User (Human) to do operation in its own context
...
For example if a User (Human) changes his name. The API Layer will pass the instruction received through the API call to the command handler for further processing.
...
For example if the User (Human) really can change his name is verified in the command validation.
