...
On new installations, the Zitadel setup job can be configured to automatically write a Personal Access Token (PAT) for the login application.
...
In that case, you can create a new PAT for the login application manually.
...
Create a machine user PAT like you created the [login application PAT above](#create-login-client), but give the user the Iam Owner role (internally called `IAM_OWNER`).
</Callout>
