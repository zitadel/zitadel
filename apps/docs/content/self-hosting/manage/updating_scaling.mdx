...
`zitadel init` performs two distinct tiers of work:

**Tier 1 — Instance provisioning** (requires a PostgreSQL superuser via `Database.postgres.Admin.Username`):
- If it doesn't exist already, it creates the unprivileged user configured in `Database.postgres.User.Username`.
...
**Tier 2 — Schema bootstrapping** (requires database owner privileges, uses the service user):
- If they don't exist already, it creates all schemas (`eventstore`, `projections`, `system`) and base tables.

:::note Manual provisioning
If you manage Tier 1 yourself (e.g. on a managed PostgreSQL service where you don't have superuser access),
you can skip Tier 1 entirely by running only the schema bootstrapping step:

```bash
zitadel init schema
```

This sub-command connects using the service user credentials and only performs Tier 2.
