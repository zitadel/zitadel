...
When you upgrade ZITADEL to a newer version, the setup phase runs database migrations automatically via `start-from-init` or `zitadel setup`.
In most cases, the migration succeeds and the container starts normally.
...
- The `start-from-init` command runs the init and setup phases on every start. Both phases are idempotent: errors like `role "zitadel" already exists` or `database "zitadel" already exists` are expected on a re-run and are handled gracefully. They do not prevent startup.
...
- For enabling the Login V2 on an existing instance, follow the [Login Client guide](/self-hosting/manage/login-client).
