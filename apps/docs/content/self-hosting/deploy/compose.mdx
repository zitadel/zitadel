...
curl -fsSLO https://raw.githubusercontent.com/zitadel/zitadel/main/deploy/compose/compose.yaml &&
...
<Callout type="info">
If you're using `podman` instead of `docker`, run `echo $XDG_RUNTIME_DIR/podman/podman.sock` & use the output as value for the variable `CONTAINER_SOCK` in your `.env` file.
</Callout>
...
|------|-------------|-------------|
| Let's Encrypt | `compose.mode-letsencrypt.yaml` | Public domain, automatic certs |
| External TLS | `compose.mode-external-tls.yaml` | Behind a load balancer, CDN, or WAF that terminates TLS |
| Local TLS | `compose.mode-local-tls.yaml` | Self-signed certs for LAN-only access |

Download the overlay you need, then start with both files:

```shell
# Download the overlay (replace with your chosen mode)
curl -fsSLO https://raw.githubusercontent.com/zitadel/zitadel/main/deploy/compose/compose.mode-letsencrypt.yaml

# Start with the overlay
docker compose --env-file .env \
  -f compose.yaml \
  -f compose.mode-letsencrypt.yaml \
  up -d --wait
```
...
```shell
docker compose --env-file .env -f compose.yaml --profile cache up -d --wait
```
...
```shell
curl -fsSLO https://raw.githubusercontent.com/zitadel/zitadel/main/deploy/compose/compose.prodlike.yaml &&

docker compose --env-file .env \
  -f compose.yaml \
  -f compose.prodlike.yaml \
  up -d --wait
```
...
docker compose --env-file .env -f compose.yaml --profile observability up -d --wait
```
