...
The `start-from-init` command used in the quickstart is idempotent: the init and setup phases are safe to re-run and will skip steps that are already complete.

1. Edit `docker-compose.yaml` and change the image tags for the `zitadel` and `login` services to the desired version, for example:
   ```yaml
   zitadel:
   ...
   ```
   Or use `latest` to always pull the newest release:
   ```yaml
   zitadel:
...
2. Pull the new images:
   ...
   docker compose pull
   ```
3. Recreate and restart the services:
   ```shell
   docker compose up --detach --wait
   ```

:::note
The `ZITADEL_FIRSTINSTANCE_*` and `ZITADEL_DEFAULTINSTANCE_*` environment variables are only applied during the **initial setup** of a new Zitadel instance.
