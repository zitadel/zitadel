...
**For Stage 2** (TLS overlays), users download the specific overlay they need the same way:

```shell
# Example: download the Let's Encrypt overlay
...
To forward traces to your own backend (Grafana Tempo, Jaeger, OpenObserve, etc.), set `OTEL_BACKEND_ENDPOINT` and uncomment the `otlp` exporter in `otel-collector-config.yaml`.
