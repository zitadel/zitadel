...
The ZITADEL chart ships with an optional in-cluster PostgreSQL subchart, so a single `helm install` deploys the full stack — no separate database install, no manual secret wiring.
...
<Callout type="info">
No cluster yet? [k3d](https://k3d.io/) is a quick way to spin one up locally — it runs k3s in Docker and includes Traefik as the ingress controller:
```bash
...
zitadel:
  ...
  configmapConfig:
    ...
    ExternalPort: 8080          # the port users will access
...
ingress:
  enabled: true
  className: traefik   # replace with your ingress controller (nginx, alb, …)
  hosts:
    - host: localhost
      paths:
        - path: /
...
login:
  ingress:
    enabled: true
    className: traefik   # must match ingress.className above
    hosts:
      - host: localhost
        paths:
          - path: /ui/v2/login
...
That's it. Visit [http://localhost:8080/ui/console](http://localhost:8080/ui/console) (adjust the host/port to match your `ExternalDomain` and `ExternalPort`) to open the console.
...
<Callout type="info">
The stack runs: **your ingress controller** ([reverse proxy](/self-hosting/manage/reverseproxy/reverse_proxy)) → **ZITADEL API** (Go) + **ZITADEL Login** (Next.js) → **PostgreSQL**.
ZITADEL's API uses [gRPC over HTTP/2](/self-hosting/manage/http2) — make sure your ingress controller forwards h2c traffic to ZITADEL pods.
</Callout>
