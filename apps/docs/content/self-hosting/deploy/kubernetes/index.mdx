...
Save the following as `quickstart-values.yaml` and run `helm install zitadel zitadel/zitadel --values quickstart-values.yaml --wait`:

```yaml
# bundled PostgreSQL — auto-wires DB connection to ZITADEL
postgresql:
  enabled: true
  auth:
    database: zitadel
...
# bundled Traefik — auto-sets ingress.className to "traefik"
# isDefaultClass: false prevents conflicts with existing ingress controllers
traefik:
  enabled: true
  ingressClass:
...
zitadel:
  ...
  configmapConfig:
    ExternalDomain: localhost   # replace with your domain/IP
    ExternalPort: 80            # use 8080 with k3d port mapping
...
# ingress.className is auto-set to "traefik" when traefik.enabled=true
ingress:
  enabled: true
  hosts:
    - host: localhost
