...
For the full configuration reference, see [Instrumentation](/self-hosting/manage/instrumentation).
...
env:
  ...
  - name: ZITADEL_INSTRUMENTATION_TRACE_FRACTION
    value: "1.0"
...
ZITADEL can expose [Prometheus](https://prometheus.io/) metrics at `/debug/metrics` or push them to an OpenTelemetry Collector.
...
env:
  - name: ZITADEL_INSTRUMENTATION_METRIC_EXPORTER_TYPE
    value: "grpc"
  - name: ZITADEL_INSTRUMENTATION_METRIC_EXPORTER_ENDPOINT
    value: "otelcol-opentelemetry-collector.monitoring.svc.cluster.local:4317"
```

When using Prometheus scraping, enable the Helm chart ServiceMonitor:
...
ZITADEL writes structured logs to stderr. Choose a format and log level:

```yaml
env:
  - name: ZITADEL_INSTRUMENTATION_LOG_FORMAT
    value: "json"
  - name: ZITADEL_INSTRUMENTATION_LOG_LEVEL
    value: "INFO"
  - name: ZITADEL_INSTRUMENTATION_LOG_STREAMS
    value: "runtime,ready,request,event_handler,queue"
```

Logs can additionally be exported to an OpenTelemetry Collector:

```yaml
env:
  - name: ZITADEL_INSTRUMENTATION_LOG_EXPORTER_TYPE
    value: "grpc"
  - name: ZITADEL_INSTRUMENTATION_LOG_EXPORTER_ENDPOINT
    value: "otelcol-opentelemetry-collector.monitoring.svc.cluster.local:4317"
```

Logs must be collected by a log collector running in your cluster. Common log collectors include:
...
For details on log fields and structure, see [Log Structure](/self-hosting/manage/log-structure).
