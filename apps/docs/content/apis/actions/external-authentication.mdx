...
This flow is executed if the User (Human) logs in using an [identity provider](/guides/integrate/identity-providers/introduction).

The flow is represented by the following Ids in the API: `FLOW_TYPE_EXTERNAL_AUTHENTICATION` and `1`

## Post Authentication

A User (Human) has authenticated externally. ZITADEL retrieved and mapped the external information.
...
A User (Human) selected **Register** on the overview page after external authentication. ZITADEL did not create the User (Human) yet.
...
- `api`
  ...
  - `setEmailVerified(bool)`  
    If true the email set is verified without User (Human) interaction
  - `setPhone(string)`  
    Sets the phone number
  - `setPhoneVerified(bool)`  
    If true the phone number set is verified without User (Human) interaction
  - `v1`
    - `user`
      - `appendMetadata(string, Any)`
        The first parameter represents the key and the second a value which will be stored
...
A User (Human) selected **Register** on the overview page after external authentication and ZITADEL successfully created the user.
