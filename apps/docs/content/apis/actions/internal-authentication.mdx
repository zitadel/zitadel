...
This flow is executed if the User (Human) logs in using the login UI hosted by ZITADEL.

The flow is represented by the following Ids in the API: `3`

## Post Authentication

A User (Human) has authenticated directly at ZITADEL.
...
A User (Human) registers directly at ZITADEL.
ZITADEL did not create the User (Human) yet.
