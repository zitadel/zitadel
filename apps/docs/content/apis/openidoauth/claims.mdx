...
| groups [^2]                                       | When requested | When requested                          | When requested                                  | When JWT and requested                               |
...
[^2]: The `groups` claim contains the user's [group](/concepts/structure/groups) memberships, requested with the `groups` scope. It follows [RFC 9068](https://www.rfc-editor.org/rfc/rfc9068.html#section-2.2.3.1) with values encoded according to [SCIM (RFC 7643)](https://www.rfc-editor.org/rfc/rfc7643#section-4.1.2): `{"groups": [{"value": "<group ID>", "display": "<group name>"}]}`.
