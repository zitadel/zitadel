...
For JavaScript/TypeScript integrations, ZITADEL provides official SDK packages that cover common frontend and Next.js server use cases:

- `@zitadel/zitadel-js`: framework-agnostic core helpers (transport/client factories, v2 client factories, token helpers, webhook verification)
- `@zitadel/nextjs`: Next.js App Router helpers for OIDC login (`auth/oidc`), custom Login UI/session flows (`auth/session`), v2 API access (`api`), and Actions v2 webhooks (`webhook`)
