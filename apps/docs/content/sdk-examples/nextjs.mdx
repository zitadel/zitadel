...
- `npx @zitadel/nextjs add` to bootstrap OIDC routes, a `/auth` test page with OIDC result output, and `.env.example` in an existing Next.js App Router project (`--auth session`, `--with-api`, and `--with-webhook` are available for explicit scaffolding)
