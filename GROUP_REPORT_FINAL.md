...
**Browser verification (2026-06-12): the Cypress groups suite passes 4/4 against a live ZITADEL with the embedded console** — create, rename, members dialog, and delete with confirmation, executed in a real browser. The run surfaced and fixed a real UI bug invisible to builds and unit tests: `cnslHasRole`'s template wrapper broke `cnsl-table-actions` content projection, so the delete/grants row buttons never rendered (row gating removed; the API enforces permissions). The spec now seeds fixtures through the group API (`tests/functional-ui/cypress/support/api/groups.ts`). The second SCIM-found defect (nil permission-check panic) and a SCIM discovery regression were also caught and fixed in this campaign.

**Live-stack verification (2026-06-12, per the make-it-work principle):** the full group v2 integration suite, the SCIM `/Groups` lifecycle, the OIDC userinfo suite, and a dedicated group-grant token-merge test were executed against a locally running ZITADEL + Postgres + Redis. All green. The projection DDL (`group_grants1`, `group_manager_roles1`, `group_users1`) applied at startup; the merge SQL ran through real token flows. The run caught and fixed one real defect: `GetGroupByID` panicked on nil permission checks (hit by SCIM, impossible to reach via gRPC). Remaining unexecuted locally: the Cypress browser suite (needs the console served against this stack) and `permission_check_v2`-flagged group manager role evaluation (documented boundary).
...
**Production-readiness verification campaign (2026-06-12, all against the live stack):**
- **Phase B authz-in-action** (`test(group): prove group manager roles through the live authorization path`): a plain machine user acting with its own PAT has zero memberships and is denied `ListOrgMembers`; after `SetGroupManagerRoles(ORG_OWNER_VIEWER)` + group join, `ListMyMemberships` returns the group-supplied org membership with the role and `ListOrgMembers` succeeds; leaving the group revokes both. The membership union arm works through the real authz path, not just SQL.
- **OD-9 concurrency + cascade edge cases** (`test(group): cover concurrent membership writes and deletion cascades`): 8 overlapping concurrent `AddUsersToGroup` calls converge to complete, duplicate-free membership with an exact `user_count`; rename to the uppercase of an existing name → `AlreadyExists`; `UserV2.DeleteUser` removes memberships and decrements counts; `OrgV2.DeleteOrganization` with groups+members+grants leaves no group state and no projection wedge. Finding (documented platform behavior, issue #7202): extreme same-aggregate contention can exhaust eventstore push retries (MaxRetries 5) and surface a retryable error to the client — not groups-specific, no partial writes.
- **SCIM hardening** (`test(scim): harden group endpoint guarantees`): duplicate displayName (same and different case) → 409 with scimType `uniqueness`; PATCH → 501 with proper SCIM error shape (`SCIM-GRP3p`, points to PUT); list pagination via count/startIndex returns every group exactly once with stable totals; filtering rejected explicitly with 501; groups fully participate in `/Bulk` (create via bulkId → 201 + Location, delete → 204).
- **SAML assertion proof** (`test(saml): assert group memberships in issued assertions`): a real signed SAMLResponse from a full auth-request → session → CreateResponse flow carries the `groups` attribute with the member's group names in the assertion XML.
- **Scale sanity** (`test(group): verify counts and paging on a 100-member group`): 100-member group reports exact `user_count`, pages exactly-once across 4 pages at 30–102ms per page, and batch removal keeps the count consistent.
- **Browser depth** (`test(console): exercise group member and grant flows in the browser`): the Cypress suite grew from 4 to 7 scenarios, all passing against the live embedded console — adding a member through `cnsl-search-user-autocomplete` with the table's user count updating after dialog close, removing a member, and creating + revoking a group grant against a seeded project/role. The run surfaced a projection race in the test seeding helpers (delete-then-recreate could hit a stale search and skip the create); the helpers now wait for projection propagation.
- **Third product defect caught and fixed** (`fix(query): stabilize group list pagination with unique sort tiebreakers`): the full-package regression rerun failed the 100-member scale test with 95 distinct members and 5 duplicates across pages. Root cause: the default sort (`creation_date`) is identical for all members added in one batch event, so Postgres was free to reorder rows between page requests — duplicates and missing rows for any paging client, including the console. Fix: `searchGroups`/`searchGroupUsers`/`searchGroupGrants` now append unique ID tiebreakers after the caller's sort (`ORDER BY <chosen>, id...`). Verified by 3 consecutive scale-test passes plus a full-package rerun; the bug only manifested under whole-package timing, which is exactly why the regression pass existed.
