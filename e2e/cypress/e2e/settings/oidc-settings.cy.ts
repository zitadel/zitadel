...
describe('oidc settings', () => {
    ...
    before(`ensure they are set`, () => {
        apiAuth().then((apiCallProperties) => {
            ensureOIDCSettingsSet(apiCallProperties, accessTokenPrecondition, idTokenPrecondition, refreshTokenExpirationPrecondition, refreshTokenIdleExpirationPrecondition);
    ...
    it(`should update oidc settings`, () => {
        ...
        cy.get('[formcontrolname="refreshTokenExpiration"]').should('value', refreshTokenExpirationPrecondition).clear().type("30");
        cy.get('[formcontrolname="refreshTokenIdleExpiration"]').should('value', refreshTokenIdleExpirationPrecondition).clear().type("7");
